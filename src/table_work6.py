...
#####################
    def create_table_toolbar(self, win):
        """Create a toolbar for table editing with enhanced styling options"""
        toolbar = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=5)
        toolbar.set_margin_start(10)
        toolbar.set_margin_end(10)
        toolbar.set_margin_top(5)
        ...
        # Border width slider - updated to allow zero
        ...
        width_scale.add_mark(0, Gtk.PositionType.BOTTOM, "None")  # Add a mark at zero
        width_scale.add_mark(1, Gtk.PositionType.BOTTOM, "")
        width_scale.add_mark(2, Gtk.PositionType.BOTTOM, "")
        width_scale.add_mark(3, Gtk.PositionType.BOTTOM, "")
        width_scale.add_mark(4, Gtk.PositionType.BOTTOM, "")
        width_scale.add_mark(5, Gtk.PositionType.BOTTOM, "")
        ...
        # Color picker button - Fix for deprecation warning
        color_button = Gtk.ColorButton()
        # Create RGBA color object first
        border_color = Gdk.RGBA()
        border_color.red = 0.8
        border_color.green = 0.8
        border_color.blue = 0.8
        border_color.alpha = 1.0
        color_button.set_rgba(border_color)  # Now use the created color object
        ...
        # Create a box for header color controls
        header_color_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=8)
        header_color_box.set_margin_top(4)
        
        # Header color button - Fix for deprecation warning
        header_color_button = Gtk.ColorButton()
        # Create RGBA color object first
        header_color = Gdk.RGBA()
        header_color.red = 0.94
        header_color.green = 0.94
        header_color.blue = 0.94
        header_color.alpha = 1.0
        header_color_button.set_rgba(header_color)  # Now use the created color object
        ...
        header_color_box.append(header_color_button)
        
        # Add a transparent button for header background
        header_transparent_button = Gtk.Button(label="Transparent")
        header_transparent_button.connect("clicked", lambda btn: self.set_header_background_transparent(win))
        header_color_box.append(header_transparent_button)
        
        fill_box.append(header_color_box)
        ...
        # Create a box for cell color controls
        cell_color_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=8)
        cell_color_box.set_margin_top(4)
        
        # Cell color button - Fix for deprecation warning
        cell_color_button = Gtk.ColorButton()
        # Create RGBA color object first
        cell_color = Gdk.RGBA()
        cell_color.red = 1.0
        cell_color.green = 1.0
        cell_color.blue = 1.0
        cell_color.alpha = 1.0
        cell_color_button.set_rgba(cell_color)  # Now use the created color object
        ...
        cell_color_box.append(cell_color_button)
        
        # Add a transparent button for cell background
        cell_transparent_button = Gtk.Button(label="Transparent")
        cell_transparent_button.connect("clicked", lambda btn: self.set_cell_background_transparent(win))
        cell_color_box.append(cell_transparent_button)
        
        fill_box.append(cell_color_box)
...
# Add these methods to your HTMLEditorApp class
    ...
    def set_cell_background_transparent(self, win):
        """Set the background of table cells to transparent"""
        # Check if we're only applying to selected cells
        selection_only = hasattr(win, 'color_selection_only') and win.color_selection_only.get_active()
        ...
        """
        self.execute_js(win, js_code)
        win.statusbar.set_text("Cell background set to transparent")

    def set_header_background_transparent(self, win):
        """Set the background of table headers to transparent"""
        # Check if we're only applying to selected cells
        selection_only = hasattr(win, 'color_selection_only') and win.color_selection_only.get_active()
        ...
        """
        self.execute_js(win, js_code)
        win.statusbar.set_text("Header background set to transparent")

    # Modify the border width setting method to allow zero
    def set_table_border_width(self, win, width):
        """Set the border width for the active table, allowing zero width"""
        js_code = f"""
        (function() {{
            if (!activeTable) return false;
            ...
            // Special case for the table element itself when width is 0
            if ({int(width)} === 0) {{
                // Also remove table border
                activeTable.style.border = '0';
                
                // Fix for border-collapse when border is 0
                if (activeTable.style.borderCollapse === 'collapse') {{
                    activeTable.style.borderCollapse = 'separate';
                    activeTable.style.borderSpacing = '0';
                }}
            }} else {{
                // For non-zero borders, restore collapse mode
...
######################
    def on_table_dialog_response(self, win, dialog, rows, cols, has_header, border_width, width_option, 
                                  border_style, border_color, add_shadow, position, z_index):
        """Handle response from the table dialog with updated styling to match the example"""
        dialog.close()
        ...
        # Convert color to hex format - using a lighter gray for borders
        color_hex = "#dddddd"  # Light gray border color
        
        # Prepare shadow CSS if needed
        shadow_css = ""
        if add_shadow:
            shadow_css = "box-shadow: 2px 2px 4px rgba(0,0,0,0.1);"
        ...
        # Execute JavaScript to insert the table with the specified styling
        js_code = f"""
        (function() {{
            // Create a table with the clean border style like the example
            let tableHTML = '<table class="{position_class}" style="border-collapse: collapse; width: {width_value}; {shadow_css} {z_index_css} border: 1px solid {color_hex};">';
            
            // Create header row if requested
            if ({str(has_header).lower()}) {{
                tableHTML += '<tr>';
                for (let j = 0; j < {cols}; j++) {{
                    tableHTML += '<th style="border: 1px solid {color_hex}; padding: 8px; background-color: #f0f0f0;">Header ' + (j+1) + '</th>';
                }}
                tableHTML += '</tr>';
            }}
            ...
            for (let i = 0; i < {rows - (1 if has_header else 0)}; i++) {{
                tableHTML += '<tr>';
                for (let j = 0; j < {cols}; j++) {{
                    tableHTML += '<td style="border: 1px solid {color_hex}; padding: 8px; min-width: 30px;">Cell</td>';
                }}
                tableHTML += '</tr>';
            }}
    ...
    def apply_color_preset(self, win, preset):
        """Apply a predefined color scheme to the table, supporting both dark and light modes"""
        # Define the preset names
        presets = ["default", "blue", "green", "warm"]
        ...
        (function() {{
            ...
            // First remove any existing preset classes
            activeTable.classList.remove('preset-default', 'preset-blue', 'preset-green', 'preset-warm');
            
            // Add the new preset class - the CSS will handle dark/light mode appropriately
            activeTable.classList.add('preset-{preset}');
            
            // Make sure the table doesn't have any inline styles that would override our class styles
            const cells = activeTable.querySelectorAll('td, th');
            for (let cell of cells) {{
                // Keep any non-color inline styles (width, height, etc.)
                const style = cell.style;
                const savedWidth = style.width;
                const savedHeight = style.height;
                const savedPadding = style.padding;
                const savedMinWidth = style.minWidth;
                const savedPosition = style.position;
                
                // Clear background and border colors (let CSS classes handle these)
                cell.style.backgroundColor = '';
                cell.style.borderColor = '';
                
                // Restore non-color styles
                if (savedWidth) style.width = savedWidth;
                if (savedHeight) style.height = savedHeight;
                if (savedPadding) style.padding = savedPadding;
                if (savedMinWidth) style.minWidth = savedMinWidth;
                if (savedPosition) style.position = savedPosition;
                
                // Remove zebra stripe class if present
                cell.classList.remove('zebra-stripe');
            }}
            
            // Clear table border color (let CSS handle it)
            activeTable.style.borderColor = '';
            ...
            // Check if we're in dark mode and log it (for debugging)
            const isDarkMode = window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches;
            console.log("Applied preset:", '{preset}', "Dark mode:", isDarkMode);
    ...
    def get_editor_html(self, content=""):
        ...
        <head>
            ...
            <style>
                ...
                /* Table styles with light borders */
                table {{
                    border-collapse: collapse;
                    margin: 10px 0;
                    position: relative;
                    ...
                    /* Ensure whole pixels for better rendering */
                    border-spacing: 0;
                }}
                
                /* Table cells with light borders */
                table td, table th {{
                    border: 1px solid #dddddd;
                    padding: 8px;
                    min-width: 30px;
                ...
                /* Fix any specific border issues */
                ...
                /* Remove special styling for the bottom left cell */
                ...
                /* Support for zero-width borders */
                table.no-borders {{
                ...
                /* Preset colors for light mode */
                table.preset-default {{
                    border-color: #dddddd;
                }}
                table.preset-default th {{
                    background-color: #f0f0f0;
                    border-color: #dddddd;
                }}
                table.preset-default td {{
                    background-color: #ffffff;
                    border-color: #dddddd;
                }}
                ...
                table.preset-blue th {{
                    background-color: #d4e6f1;
                    border-color: #aed6f1;
                }}
                table.preset-blue td {{
                ...
                table.preset-green th {{
                    background-color: #d4efdf;
                    border-color: #a9dfbf;
                }}
                table.preset-green td {{
                ...
                table.preset-warm th {{
                    background-color: #f9e79f;
                    border-color: #f7dc6f;
                }}
                table.preset-warm td {{
    ...
    def insert_table_js(self):
        """JavaScript for insert table and related functionality with clean border styling"""
        return """
            // Function to insert a table at the current cursor position
            function insertTable(rows, cols, hasHeader, borderWidth, tableWidth) {
                // Create table HTML with clean styling
                let tableHTML = '<table class="no-wrap preset-default" style="border-collapse: collapse; width: ' + tableWidth + ';">';
            ...
            function addTableColumn(tableElement, position) {
                ...
                for (let i = 0; i < rows.length; i++) {
                    ...
                    // If it's a header row, make new cell a header too
