...
##############
    ...
    def table_event_handlers_js(self):
        ...
        """            
######################
# Replace these methods in your HTMLEditorApp class:

    def table_row_column_js(self):
        """JavaScript for table row and column operations with improved resizing functionality"""
        return """
        // Function to add a row to the table
        function addTableRow(tableElement, position) {
            if (!tableElement && activeTable) {
            ...
            // Re-initialize resizers after adding a row
            if (tableElement === activeTable) {
        ...
        function addTableColumn(tableElement, position) {
            ...
            // Re-initialize resizers after adding a column
            if (tableElement === activeTable) {
                initializeTableResizers(tableElement);
            }
        ...
        function deleteTableRow(tableElement, rowIndex) {
            ...
            // Re-initialize resizers after deleting a row
            if (tableElement === activeTable) {
                initializeTableResizers(tableElement);
            }
        ...
        function deleteTableColumn(tableElement, colIndex) {
            ...
            // Re-initialize resizers after deleting a column
            if (tableElement === activeTable) {
                initializeTableResizers(tableElement);
            }
        ...
        // Function to initialize row/column resizing for the entire table
        function initializeTableResizers(tableElement) {
            ...
            // Add column resizers to each cell (except the last column)
            for (let i = 0; i < tableElement.rows.length; i++) {
                const row = tableElement.rows[i];
                for (let j = 0; j < row.cells.length - 1; j++) {  // Don't add to last column
            ...
            // Add row resizers to each cell (except the last row)
            for (let i = 0; i < tableElement.rows.length - 1; i++) {  // Don't add to last row
                const row = tableElement.rows[i];
        ...
        function updateColumnWidth(tableElement, columnIndex, newWidth) {
            ...
            // Also update the next column to maintain table width
            for (let i = 0; i < tableElement.rows.length; i++) {
                const nextCell = tableElement.rows[i].cells[columnIndex + 1];
                if (nextCell) {
                    nextCell.style.width = 'auto';
                }
            }
    ...
    def table_handles_css_js(self):
        ...
        return """
            ...
            /* Column resizer - appears on right edge of each cell except last column */
            .column-resizer {
                position: absolute;
                right: -4px;
                top: 0;
            ...
            /* Row resizer - appears on bottom edge of each cell except last row */
            .row-resizer {
                position: absolute;
                bottom: -4px;
                left: 0;
            ...
            table td, table th {
                position: relative;
                overflow: visible; /* Ensure resizers are not clipped */
            }
