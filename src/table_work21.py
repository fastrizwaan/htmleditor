...
################
    ...
    def _create_border_tab(self, win, popover):
        ...
        border_box.append(primary_border_box)
    ...
    def _create_margin_tab(self, win, popover):
        """Create the margin properties tab"""
        ...
        # Define positions for a visual layout
        positions = {
            'top': (0, 1),
            'left': (1, 0),
            'right': (1, 2),
        ...
        for side, (row, col) in positions.items():
            ...
            # Create spin button
            adjustment = Gtk.Adjustment(value=5, lower=0, upper=100, step_increment=1)
            spin = Gtk.SpinButton()
    ...
    def _create_color_tab(self, win, popover):
        """Create the color properties tab"""
        ...
        # Border color row
        border_color_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=8)
        border_color_label = Gtk.Label(label="Border Color:")
        border_color_label.set_halign(Gtk.Align.START)
        border_color_label.set_hexpand(True)
        
        border_color_button = Gtk.ColorButton()
        border_color_button.connect("color-set", lambda btn: self._apply_border_color(win, btn))
        
        border_color_box.append(border_color_label)
        border_color_box.append(border_color_button)
        color_box.append(border_color_box)
        
        # Table background color row
        table_color_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=8)
        table_color_label = Gtk.Label(label="Table Color:")
        table_color_label.set_halign(Gtk.Align.START)
        table_color_label.set_hexpand(True)
        
        table_color_button = Gtk.ColorButton()
        table_color_button.connect("color-set", lambda btn: self._apply_table_color(win, btn))
        
        table_color_box.append(table_color_label)
        table_color_box.append(table_color_button)
        color_box.append(table_color_box)
        
        # Header color row
        header_color_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=8)
        header_color_label = Gtk.Label(label="Header Color:")
        header_color_label.set_halign(Gtk.Align.START)
        header_color_label.set_hexpand(True)
        
        header_color_button = Gtk.ColorButton()
        header_color_button.connect("color-set", lambda btn: self._apply_header_color(win, btn))
        
        header_color_box.append(header_color_label)
        ...
        cell_color_label = Gtk.Label(label="Cell Color:")
        cell_color_label.set_halign(Gtk.Align.START)
        cell_color_label.set_hexpand(True)
        
        cell_color_button = Gtk.ColorButton()
        cell_color_button.connect("color-set", lambda btn: self._apply_cell_color(win, btn))
        
        cell_color_box.append(cell_color_label)
    ...
    def on_table_button_clicked(self, win, button):
        ...
        content_box.set_size_request(400, 300)  # Ensure good size for tabs
        ...
        <head>
            ...
            <style>
                ...
                table.center-align {{
                    margin-left: auto;
                    margin-right: auto;
                    margin-top: 0;  /* Ensure no top margin for centered tables */
                    float: none;
                    clear: both;
                }}
                table.no-wrap {{
    ...
    def _show_color_dialog(self, win, color_dialog, button):
        ...
        # Use native dialog with parent window
        color_dialog.choose_rgba(win, None, None, 
                                 lambda dialog, result, data: self._on_color_chosen(dialog, result, data),
                                 win)
        return False  # Don't call again
    ...
    def _on_color_chosen(self, dialog, result, win):
    ...
    def _apply_border_color(self, win, color_button):
        """Apply border color from color button"""
    ...
    def _apply_table_color(self, win, color_button):
        """Apply table background color from color button"""
        rgba = color_button.get_rgba()
        hex_color = f"#{int(rgba.red * 255):02x}{int(rgba.green * 255):02x}{int(rgba.blue * 255):02x}"
        
        js_code = f"""
        (function() {{
            setTableBackgroundColor('{hex_color}');
            return true;
        }})();
        """
        self.execute_js(win, js_code)
    ...
    def _apply_header_color(self, win, color_button):
        """Apply header background color from color button"""
        rgba = color_button.get_rgba()
        hex_color = f"#{int(rgba.red * 255):02x}{int(rgba.green * 255):02x}{int(rgba.blue * 255):02x}"
        
        js_code = f"""
        (function() {{
            setHeaderBackgroundColor('{hex_color}');
            return true;
        }})();
        """
        self.execute_js(win, js_code)
    ...
    def _apply_cell_color(self, win, color_button):
        """Apply cell background color from color button"""
        rgba = color_button.get_rgba()
        hex_color = f"#{int(rgba.red * 255):02x}{int(rgba.green * 255):02x}{int(rgba.blue * 255):02x}"
        
        js_code = f"""
        (function() {{
            setCellBackgroundColor('{hex_color}');
            return true;
        }})();
        """
        self.execute_js(win, js_code)
    ...
    def table_color_js(self):
        """JavaScript for table color operations"""
        return """
        ...
        function setCellBackgroundColor(color) {
            ...
            activeTable.setAttribute('data-cell-color', color);
        ...
        function getTableColors() {
            ...
            // Try to get from stored attributes first
            const bgColor = activeTable.getAttribute('data-bg-color');
            const headerColor = activeTable.getAttribute('data-header-color');
            const cellColor = activeTable.getAttribute('data-cell-color');
            const borderColor = activeTable.getAttribute('data-border-color');
            
            // If stored values exist, return them
            if (bgColor || headerColor || cellColor || borderColor) {
                return {
                    background: bgColor || '',
                    header: headerColor || '',
                    cell: cellColor || '',
                    border: borderColor || getBorderColor()
                };
            }
            
            // Otherwise, get from styles
            const firstHeader = activeTable.querySelector('th');
            const firstCell = activeTable.querySelector('td');
            
            return {
                background: activeTable.style.backgroundColor || '',
                header: firstHeader ? firstHeader.style.backgroundColor : getHeaderBgColor(),
                cell: firstCell ? firstCell.style.backgroundColor : '',
                border: firstCell ? (firstCell.style.borderColor || getBorderColor()) : getBorderColor()
            };
        }
        """
        ...
        try:
            ...
            if result_str:
                ...
                if notebook:
                    ...
                    if properties.get('margins') and hasattr(margin_page, 'margin_controls'):
                        for side in ['top', 'right', 'bottom', 'left']:
                            if side in margin_page.margin_controls and side in properties['margins']:
                                margin_page.margin_controls[side].set_value(properties['margins'][side])
                    
                    # Initialize color controls
                    if properties.get('colors'):
                        colors = properties['colors']
                        
                        # Helper function to set color button
                        def set_color_button(button, color_str):
                            if color_str:
                                try:
                                    rgba = Gdk.RGBA()
                        ...
                        set_color_button(color_page.border_color_button, colors.get('border', ''))
                        set_color_button(color_page.table_color_button, colors.get('background', ''))
                        set_color_button(color_page.header_color_button, colors.get('header', ''))
                        set_color_button(color_page.cell_color_button, colors.get('cell', ''))
