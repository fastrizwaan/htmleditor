...
################
    ...
    def get_editor_html(self, content=""):
        ...
        """##################
    ## Step 1: Add a checkbox button to the file toolbar

    # Find this section in create_file_toolbar method and add the checkbox button
    def create_file_toolbar(self, win):
        """Create the file toolbar with linked button groups"""
        file_toolbar = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=1)
        file_toolbar.set_margin_start(4)
        ...
        # Insert checkbox list button (NEW)
        checkbox_button = Gtk.Button(icon_name="object-select-symbolic")
        ...
        insert_group.append(image_button)
        insert_group.append(link_button)
        insert_group.append(checkbox_button)  # Add new checkbox button
        # Add insert group to toolbar
        file_toolbar.append(insert_group)
    ...
    ## Step 2: Add the checkbox handler method

    def on_insert_checkbox_clicked(self, win, btn):
        """Handle checkbox list insertion button click"""
        win.statusbar.set_text("Inserting checkbox list...")
        
        # Execute JavaScript to insert a checkbox list at the cursor position
        js_code = """
        (function() {
            insertCheckboxList();
            return true;
    ...
    ## Step 3: Add checkbox JavaScript to the get_editor_js method
    ...
    def get_editor_js(self):
        ...
        {self.insert_image_js()}
        {self.insert_link_js()}
        {self.insert_checkbox_js()}
        {self.init_editor_js()}
        """

    ## Step 4: Create the checkbox JavaScript implementation

    def insert_checkbox_js(self):
        """JavaScript for checkbox list and related functionality"""
        return """
        // Function to insert a checkbox list at the current cursor position
        function insertCheckboxList() {
            // Create a new UL element with the checkbox-list class
            const html = '<ul class="checkbox-list"><li><span class="checkbox unchecked">☐</span> </li></ul>';
            
            // Insert the HTML at the current cursor position
            document.execCommand('insertHTML', false, html);
            
            // After inserting, add event listeners to the checkboxes
            setupCheckboxListeners();
            
            // Set focus to the empty item
            const items = document.querySelectorAll('.checkbox-list li');
            if (items.length > 0) {
                ...
                // Find the text node after the checkbox, or create one if needed
                let textNode = null;
                for (let i = 0; i < lastItem.childNodes.length; i++) {
                    if (lastItem.childNodes[i].nodeType === 3) { // Text node
                        textNode = lastItem.childNodes[i];
                        break;
                    }
                }
        ...
        // Function to toggle a checkbox state between checked and unchecked
        ...
        // Function to find all checkbox lists in the document and set up event listeners
        function setupCheckboxListeners() {
            const editor = document.getElementById('editor');
            const checkboxes = editor.querySelectorAll('.checkbox-list .checkbox');
            ...
            // Set up keydown handlers for the entire editor
            if (!editor.hasAttribute('data-checkbox-handlers')) {
                editor.setAttribute('data-checkbox-handlers', 'true');
                editor.addEventListener('keydown', handleCheckboxKeydown);
            }
        }
        ...
        // Function to handle keydown events in checkbox lists
        function handleCheckboxKeydown(event) {
            // Only handle Enter and backspace for now
            ...
            // Find if we're inside a checkbox list item
            let listItem = null;
            ...
            while (node && node !== document.body) {
                if (node.nodeName === 'LI') {
                    listItem = node;
                } else if (node.nodeName === 'UL' && node.classList.contains('checkbox-list')) {
                    checkboxList = node;
                    break;
                }
            ...
            // If we're not in a checkbox list, do nothing
            if (!checkboxList || !listItem) return;
            
            // Check if the current list item is empty (apart from the checkbox)
            ...
            if (event.key === 'Enter') {
                ...
                if (isEmpty && hasEmptyListItemBefore(listItem)) {
                    // Two empty list items in a row - exit the list
                    exitCheckboxList(listItem);
                } else if (isEmpty) {
                    // Empty item - mark it as a placeholder for potential exit next time
                    listItem.setAttribute('data-empty', 'true');
                } else {
                    // Normal case - insert new list item with checkbox
                    insertNewCheckboxItem(listItem);
                }
            ...
            if (event.key === 'Backspace') {
                // Check if cursor is at the beginning of the list item (right after the checkbox)
                const isAtStart = isSelectionAtStartOfListItem(listItem);
                
                if (isAtStart) {
                    // Prevent the default behavior which would delete the checkbox
                    event.preventDefault();
                    
                    // If there's a previous list item, we'll move up to it
        ...
        // Check if a list item is empty (contains only the checkbox)
        function isListItemEmpty(listItem) {
            let isEmpty = true;
            ...
            for (let i = 0; i < listItem.childNodes.length; i++) {
                ...
                // Skip the checkbox span
                if (node.nodeType === 1 && node.classList && node.classList.contains('checkbox')) {
                    foundCheckbox = true;
                    continue;
                }
                ...
                if (node.nodeType === 3 && node.nodeValue.trim() !== '') {
                    isEmpty = false;
                    break;
                } else if (node.nodeType === 1 && !isElementEmpty(node)) {
                    isEmpty = false;
                    break;
                }
            }
            
            return isEmpty && foundCheckbox;
        }
        
        // Helper to check if an element is empty
        function isElementEmpty(element) {
            return element.textContent.trim() === '';
        }
        
        // Check if the previous list item is empty
        function hasEmptyListItemBefore(listItem) {
            const prevItem = listItem.previousElementSibling;
            if (prevItem && prevItem.nodeName === 'LI') {
                return isListItemEmpty(prevItem) || prevItem.hasAttribute('data-empty');
            }
            return false;
        }
        
        // Function to exit the checkbox list
        function exitCheckboxList(listItem) {
            ...
            // If the previous item is also empty, remove it too
            const prevItem = list.lastElementChild;
            if (prevItem && prevItem.nodeName === 'LI' && 
                (isListItemEmpty(prevItem) || prevItem.hasAttribute('data-empty'))) {
        ...
        // Function to insert a new checkbox list item
        function insertNewCheckboxItem(currentItem) {
            ...
            focusNode(newItem, 2); // Position after the checkbox and space
        }
        
        // Helper to set focus to a node with optional offset
        function focusNode(node, offset) {
            const range = document.createRange();
            const sel = window.getSelection();
            
            if (offset !== undefined) {
                // Try to find the text node to position correctly
                let textNode = null;
        ...
        // Check if selection is right after the checkbox
        function isSelectionAtStartOfListItem(listItem) {
            const selection = window.getSelection();
            ...
            // If selection is in a text node right after the checkbox
            if (range.startContainer.nodeType === 3 && 
                range.startOffset === 0 && 
                range.startContainer.previousSibling === checkbox) {
                return true;
            }
        ...
        // Function to convert a regular list to a checkbox list
        function convertToCheckboxList(list) {
            if (!list) return;
            
            // Add the checkbox-list class
            ...
            items.forEach(item => {
                ...
                if (!item.querySelector('.checkbox')) {
                    ...
                    if (item.firstChild) {
                        ...
                        item.insertBefore(document.createTextNode(' '), item.childNodes[1]);
                    } else {
        ...
        // Function to convert a checkbox list to a regular list
        function convertToRegularList(list) {
            if (!list) return;
            
            // Remove the checkbox-list class
            list.classList.remove('checkbox-list');
            ...
            checkboxes.forEach(checkbox => {
                ...
                // If the next node is a space (text node), remove it too
        ...
        // Function to toggle list between checkbox and regular
        function toggleListType(list) {
            if (!list) return;
            
            if (list.classList.contains('checkbox-list')) {
                convertToRegularList(list);
            } else {
                convertToCheckboxList(list);
            }
        ...
        // Setup checkboxes when the document is loaded
        document.addEventListener('DOMContentLoaded', function() {
            setupCheckboxListeners();
                    ...
                    if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
                        for (let i = 0; i < mutation.addedNodes.length; i++) {
                            ...
                            if (node.nodeType === 1) { // Element node
                                if (node.classList && node.classList.contains('checkbox-list') ||
                                    node.querySelectorAll('.checkbox-list').length > 0) {
    ...
    ## Step 5: Update the get_editor_html method to include checkbox styles
