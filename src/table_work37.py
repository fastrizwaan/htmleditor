...
###################
    def table_handles_css_js_fixed(self):
        ...
        return """
            ...
            /* Table edge resizers - only right and bottom */
            .table-edge-resizer {
                position: absolute;
                background: transparent;
                z-index: 15;
    ...
    def table_row_column_js_fixed(self):
        ...
        function initializeTableResizers(tableElement) {
            ...
            // Add edge resizers for the table itself - only right and bottom
            // Right edge resizer (for table width)
            const rightEdgeResizer = document.createElement('div');
            rightEdgeResizer.className = 'table-edge-resizer right-edge';
            tableElement.appendChild(rightEdgeResizer);
        ...
        function updateColumnWidth(tableElement, columnIndex, newWidth) {
            ...
            // Ensure minimum width of 15px for columns (changed from 30px)
            const minWidth = 15;
            newWidth = Math.max(minWidth, newWidth);
        ...
        // Function to resize table from edges - only right and bottom
        function resizeTableFromEdge(tableElement, edge, deltaX, deltaY) {
            if (!tableElement) return;
    ...
    def table_insert_functions_js(self):
        ...
        function insertTable(rows, cols, hasHeader, borderWidth, tableWidth, isFloating) {
            ...
            if (hasHeader) {
                tableHTML += '<tr>';
                for (let j = 0; j < cols; j++) {
                    tableHTML += '<th style="border: ' + borderWidth + 'px solid ' + borderColor + '; padding: 5px; background-color: ' + headerBgColor + '; min-width: 30px;">Header ' + (j+1) + '</th>';
                }
                tableHTML += '</tr>';
                rows--; // Reduce regular rows by one since we added a header
            }
    ...
    def table_row_column_js_fixed(self):
        ...
        function addTableRow(tableElement, position) {
            ...
            if (rows.length > 0) {
                ...
                for (let i = 0; i < rows[0].cells.length; i++) {
                    ...
                    // Ensure minimum width for new cells
        ...
        function addTableColumn(tableElement, position) {
            ...
            for (let i = 0; i < rows.length; i++) {
                ...
                let cellStyle = {
                    border: '1px solid ' + borderColor,
                    padding: '5px',
                    minWidth: '30px'  // Ensure minimum width
                };
        ...
        function updateColumnWidth(tableElement, columnIndex, newWidth) {
            ...
            // Ensure minimum width of 30px for all columns
            const minWidth = 30;
            newWidth = Math.max(minWidth, newWidth);
            ...
            // Make sure table has proper layout
            tableElement.style.tableLayout = 'fixed';
...
########
    def table_row_column_js_fixed(self):
        ...
        // Function to update column width using simple equal distribution
        function updateColumnWidth(tableElement, columnIndex, newWidth) {
            if (!tableElement) return;
            ...
            for (let i = 0; i < columnCount; i++) {
                currentWidths.push(tableElement.rows[0].cells[i].offsetWidth);
            }
            
            // Calculate the requested change in width
            const originalWidth = currentWidths[columnIndex];
            const requestedWidth = Math.max(minWidth, newWidth);
            const widthDifference = requestedWidth - originalWidth;
            
            // Calculate how much space is available in other columns
            let availableSpace = 0;
            let adjustableColumns = 0;
            
            for (let i = 0; i < columnCount; i++) {
                if (i !== columnIndex) {
                    availableSpace += currentWidths[i] - minWidth;
                    adjustableColumns++;
                }
            }
            
            // If not enough space, limit the resize
            let actualNewWidth = requestedWidth;
            if (widthDifference > availableSpace) {
                actualNewWidth = originalWidth + availableSpace;
            }
            
            // Update the resizing column
            for (let i = 0; i < tableElement.rows.length; i++) {
                const cell = tableElement.rows[i].cells[columnIndex];
                if (cell) {
                    cell.style.width = actualNewWidth + 'px';
                    cell.style.minWidth = minWidth + 'px';
                }
            }
            
            // Calculate adjustment for other columns
            const actualDifference = actualNewWidth - originalWidth;
            if (actualDifference !== 0 && adjustableColumns > 0) {
                // Distribute the difference equally among other columns
                const adjustmentPerColumn = actualDifference / adjustableColumns;
                
                for (let i = 0; i < columnCount; i++) {
                    if (i !== columnIndex) {
                        let newColumnWidth = currentWidths[i] - adjustmentPerColumn;
                        // Ensure we don't go below minimum
                        newColumnWidth = Math.max(minWidth, newColumnWidth);
                        
                        for (let j = 0; j < tableElement.rows.length; j++) {
                            const cell = tableElement.rows[j].cells[i];
                            if (cell) {
        ...
        function resizeTableFromEdge(tableElement, edge, deltaX, deltaY) {
            ...
            switch(edge) {
                case 'right':
                    ...
                    const columnCount = tableElement.rows[0].cells.length;
                    const columnWidth = Math.max(30, Math.floor(newWidth / columnCount));
