...
class HTMLEditorApp(Adw.Application):
    ...
    def create_window(self):
        ...
        # Create formatting toolbar with animation support
        win.formatting_toolbar_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL)
        win.formatting_toolbar_box.set_halign(Gtk.Align.FILL)
        win.formatting_toolbar_box.set_valign(Gtk.Align.START)
        win.formatting_toolbar_box.add_css_class("formatting-toolbar")
        
        # Create and add the actual formatting toolbar
        win.formatting_toolbar = self.create_formatting_toolbar(win)
        win.formatting_toolbar_box.append(win.formatting_toolbar)
        
        # Add formatting toolbar box directly below header bar
        win.main_box.append(win.formatting_toolbar_box)
        
        # Create content box for webview and other elements
        ...
        try:
            ...
            # Register content changed handler
            ...
            # Register formatting update handler
            user_content_manager.register_script_message_handler("formattingUpdate")
            user_content_manager.connect("script-message-received::formattingUpdate", 
                                         lambda mgr, res: self.on_formatting_update(win, mgr, res))
        ...
        # Setup the editing toolbar animation (Ctrl+T)
        ...
        self.setup_toolbar_css(win)
        
        return win


    def setup_toolbar_css(self, win):
        """Set up CSS for toolbar styling and animations"""
    ...
    def create_formatting_toolbar(self, win):
        """Create a formatting toolbar with text styling options"""
        ...
        paragraph_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=2)
        
        paragraph_label = Gtk.Label(label="Paragraph:")
        paragraph_label.set_margin_end(5)
        paragraph_box.append(paragraph_label)
        
        paragraph_dropdown = Gtk.DropDown()
        paragraph_model = Gtk.StringList()
        for style in ["Normal", "Heading 1", "Heading 2", "Heading 3", "Heading 4", "Heading 5", "Heading 6"]:
            paragraph_model.append(style)
        
        paragraph_dropdown.set_model(paragraph_model)
        paragraph_dropdown.set_selected(0)  # Default to "Normal"
        paragraph_dropdown.connect("notify::selected", lambda dropdown, param: self.on_paragraph_style_changed(win, dropdown))
        paragraph_dropdown.set_tooltip_text("Change paragraph style")
        paragraph_box.append(paragraph_dropdown)
        
        toolbar.append(paragraph_box)
        win.paragraph_dropdown = paragraph_dropdown  # Store for access
        ...
        # Common system fonts
        system_fonts = [
            "Default", "Arial", "Helvetica", "Times New Roman", "Times", "Courier New",
            "Courier", "Verdana", "Georgia", "Palatino", "Garamond", "Bookman",
            "Tahoma", "Trebuchet MS", "Arial Black", "Impact", "Comic Sans MS"
        ]
        
        # Add additional system fonts from Pango (if available)
        try:
            import gi
            ...
            # Get all font family names and sort them
            all_fonts = sorted([family.get_name() for family in font_families])
            
            # Add system fonts first, then all Pango fonts
            ...
            print(f"Error loading Pango fonts: {e}")
            # Fallback to just the common system fonts
            for font in system_fonts:
                font_model.append(font)
        
        font_dropdown.set_model(font_model)
        font_dropdown.set_selected(0)  # Default font
        font_dropdown.connect("notify::selected", lambda dropdown, param: self.on_font_family_changed(win, dropdown))
        font_dropdown.set_tooltip_text("Change font family")
        font_box.append(font_dropdown)
        
        toolbar.append(font_box)
        win.font_dropdown = font_dropdown  # Store for access
        
        # Separator
        separator2 = Gtk.Separator(orientation=Gtk.Orientation.VERTICAL)
        separator2.set_margin_start(8)
        separator2.set_margin_end(8)
        toolbar.append(separator2)
        
        # --- Font Size Dropdown ---
        size_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=2)
        
        size_label = Gtk.Label(label="Size:")
        size_label.set_margin_end(5)
        size_box.append(size_label)
        
        size_dropdown = Gtk.DropDown()
        size_model = Gtk.StringList()
        
        # WebKit sizes 1-7 with descriptive labels
        font_sizes = ["Default", "1 (smallest)", "2 (smaller)", "3 (small)", 
                      "4 (medium)", "5 (large)", "6 (larger)", "7 (largest)"]
        
        for size in font_sizes:
            size_model.append(size)
        
        size_dropdown.set_model(size_model)
        size_dropdown.set_selected(0)  # Default to "Default"
        size_dropdown.connect("notify::selected", lambda dropdown, param: self.on_font_size_changed(win, dropdown))
        size_dropdown.set_tooltip_text("Change font size")
        size_box.append(size_dropdown)
        
        toolbar.append(size_box)
        win.size_dropdown = size_dropdown  # Store for access
        
        # Separator
        separator3 = Gtk.Separator(orientation=Gtk.Orientation.VERTICAL)
        separator3.set_margin_start(8)
        separator3.set_margin_end(8)
        toolbar.append(separator3)
        
        # --- Formatting Buttons ---
        formatting_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
        
        # Bold button
        bold_button = Gtk.Button(icon_name="format-text-bold-symbolic")
        bold_button.set_tooltip_text("Bold (Ctrl+B)")
        bold_button.connect("clicked", lambda btn: self.on_bold_clicked(win, btn))
        formatting_box.append(bold_button)
        
        # Italic button
        italic_button = Gtk.Button(icon_name="format-text-italic-symbolic")
        italic_button.set_tooltip_text("Italic (Ctrl+I)")
        italic_button.connect("clicked", lambda btn: self.on_italic_clicked(win, btn))
        formatting_box.append(italic_button)
        
        # Underline button
        underline_button = Gtk.Button(icon_name="format-text-underline-symbolic")
        underline_button.set_tooltip_text("Underline (Ctrl+U)")
        underline_button.connect("clicked", lambda btn: self.on_underline_clicked(win, btn))
        formatting_box.append(underline_button)
        
        # Strikethrough button
        strikethrough_button = Gtk.Button(icon_name="format-text-strikethrough-symbolic")
        strikethrough_button.set_tooltip_text("Strikethrough")
        strikethrough_button.connect("clicked", lambda btn: self.on_strikethrough_clicked(win, btn))
        formatting_box.append(strikethrough_button)
        
        toolbar.append(formatting_box)
        
        return toolbar

    # Format toolbar toggle
    def toggle_formatting_toolbar(self, win, *args):
        """Toggle formatting toolbar visibility with Ctrl+Shift+F"""
        if hasattr(win, 'formatting_toolbar_box'):
            visible = win.formatting_toolbar_box.get_visible()
            win.formatting_toolbar_box.set_visible(not visible)
            status = "hidden" if visible else "shown" 
            win.statusbar.set_text(f"Formatting toolbar {status} (Ctrl+Shift+F)")
        return True

    # Format shortcut handlers
    def on_bold_shortcut(self, win, *args):
        """Handle Ctrl+B shortcut for bold formatting"""
        self.on_bold_clicked(win, None)
        return True

    def on_italic_shortcut(self, win, *args):
        """Handle Ctrl+I shortcut for italic formatting"""
        self.on_italic_clicked(win, None)
        return True

    def on_underline_shortcut(self, win, *args):
        """Handle Ctrl+U shortcut for underline formatting"""
        self.on_underline_clicked(win, None)
        return True

    # Format button handlers
    def on_bold_clicked(self, win, button):
        """Apply bold formatting to selected text or at current caret position"""
        self.execute_js(win, "document.execCommand('bold', false, null);")
        win.statusbar.set_text("Applied bold formatting")

    def on_italic_clicked(self, win, button):
        """Apply italic formatting to selected text or at current caret position"""
        self.execute_js(win, "document.execCommand('italic', false, null);")
        win.statusbar.set_text("Applied italic formatting")

    def on_underline_clicked(self, win, button):
        """Apply underline formatting to selected text or at current caret position"""
        self.execute_js(win, "document.execCommand('underline', false, null);")
        win.statusbar.set_text("Applied underline formatting")

    def on_strikethrough_clicked(self, win, button):
        """Apply strikethrough formatting to selected text or at current caret position"""
        self.execute_js(win, "document.execCommand('strikeThrough', false, null);")
        win.statusbar.set_text("Applied strikethrough formatting")

    # Dropdown handlers
    def on_paragraph_style_changed(self, win, dropdown):
        """Apply paragraph style formatting based on dropdown selection"""
        selected = dropdown.get_selected()
        model = dropdown.get_model()
        style = model.get_string(selected) if selected >= 0 else "Normal"
        
        # Map styles to HTML format tags
        style_map = {
            "Normal": "p",
            "Heading 1": "h1",
            "Heading 2": "h2",
        ...
        format_tag = style_map.get(style, "p")
        self.execute_js(win, f"document.execCommand('formatBlock', false, '<{format_tag}>');")
        win.statusbar.set_text(f"Applied {style} style")

    def on_font_family_changed(self, win, dropdown):
        """Apply font family formatting based on dropdown selection"""
        selected = dropdown.get_selected()
        model = dropdown.get_model()
        font = model.get_string(selected) if selected >= 0 else "Default"
        
        if font == "Default":
            # Remove font family formatting
            self.execute_js(win, "document.execCommand('removeFormat', false, 'fontName');")
            win.statusbar.set_text("Reset to default font")
        else:
            # Apply selected font
            self.execute_js(win, f"document.execCommand('fontName', false, '{font}');")
            win.statusbar.set_text(f"Applied {font} font")

    def on_font_size_changed(self, win, dropdown):
        """Apply font size formatting based on dropdown selection"""
        selected = dropdown.get_selected()
        model = dropdown.get_model()
        size_text = model.get_string(selected) if selected >= 0 else "Default"
        
        # Get the numeric value from the selection (assuming format "N (description)")
        if size_text == "Default":
            # Remove font size formatting
            self.execute_js(win, "document.execCommand('removeFormat', false, 'fontSize');")
            win.statusbar.set_text("Reset to default font size")
        else:
            # Extract the numeric value
            size = size_text.split(" ")[0]
            self.execute_js(win, f"document.execCommand('fontSize', false, '{size}');")
            win.statusbar.set_text(f"Applied font size {size}")
    ...
    def setup_keyboard_shortcuts(self, win):
        ...
        # Create Ctrl+T shortcut for toggling the editing toolbar
        ...
        # Create Ctrl+Shift+F shortcut for toggling the formatting toolbar
        trigger_format = Gtk.ShortcutTrigger.parse_string("<Control><Shift>f")
        action_format = Gtk.CallbackAction.new(lambda *args: self.toggle_formatting_toolbar(win, *args))
        shortcut_format = Gtk.Shortcut.new(trigger_format, action_format)
        controller.add_shortcut(shortcut_format)
        
        # Create Ctrl+B shortcut for bold
        ...
        # Create Ctrl+I shortcut for italic
        ...
        # Create Ctrl+U shortcut for underline
    ...
    def setup_toolbar_animation(self, win):
        """Set up the overlay toolbar with fade animation"""
        ...
        win.overlay_toolbar.set_margin_start(12)
        win.overlay_toolbar.set_margin_end(12)
        win.overlay_toolbar.set_margin_top(8)
        win.overlay_toolbar.set_margin_bottom(8)
        ...
        paragraph_label = Gtk.Label(label="Paragraph:")
        paragraph_label.set_margin_end(5)
        win.overlay_toolbar.append(paragraph_label)
        ...
        paragraph_dropdown.connect("notify::selected", lambda dropdown, param: self.on_paragraph_style_changed(win, dropdown))
        paragraph_dropdown.set_tooltip_text("Change paragraph style")
        win.overlay_toolbar.append(paragraph_dropdown)
        win.paragraph_dropdown_overlay = paragraph_dropdown
        
        # Add separator
        separator1 = Gtk.Separator(orientation=Gtk.Orientation.VERTICAL)
        separator1.set_margin_start(8)
        separator1.set_margin_end(8)
        win.overlay_toolbar.append(separator1)
        
        # Font Family Dropdown
        font_label = Gtk.Label(label="Font:")
        font_label.set_margin_end(5)
        win.overlay_toolbar.append(font_label)
        ...
        font_dropdown.connect("notify::selected", lambda dropdown, param: self.on_font_family_changed(win, dropdown))
        font_dropdown.set_tooltip_text("Change font family")
        win.overlay_toolbar.append(font_dropdown)
        win.font_dropdown_overlay = font_dropdown
        
        # Add separator
        separator2 = Gtk.Separator(orientation=Gtk.Orientation.VERTICAL)
        separator2.set_margin_start(8)
        separator2.set_margin_end(8)
        win.overlay_toolbar.append(separator2)
        
        # Font Size Dropdown
        size_label = Gtk.Label(label="Size:")
        size_label.set_margin_end(5)
        win.overlay_toolbar.append(size_label)
        
        size_dropdown = Gtk.DropDown()
        size_model = Gtk.StringList()
        
        # WebKit sizes 1-7
        font_sizes = ["Default", "1", "2", "3", "4", "5", "6", "7"]
        ...
        size_dropdown.connect("notify::selected", lambda dropdown, param: self.on_font_size_changed(win, dropdown))
        size_dropdown.set_tooltip_text("Change font size")
        win.overlay_toolbar.append(size_dropdown)
        win.size_dropdown_overlay = size_dropdown
        
        # Add separator
        separator3 = Gtk.Separator(orientation=Gtk.Orientation.VERTICAL)
        separator3.set_margin_start(8)
        separator3.set_margin_end(8)
        win.overlay_toolbar.append(separator3)
        ...
        strikethrough_button.set_tooltip_text("Strikethrough")
        strikethrough_button.connect("clicked", lambda btn: self.on_strikethrough_clicked(win, btn))
        win.overlay_toolbar.append(strikethrough_button)
        ...
        # Set initial state: hidden with zero opacity (starts hidden, toggle with Ctrl+T)
    ...
    <head>
        ...
        <style>
            ...
            /* Format toolbar indicator styles */
            #editor .format-active {{
    ...
    def format_text_js(self):
        """JavaScript to handle text formatting for both selection and caret."""
        return """
        // Function to ensure formatting applies correctly to both
        // selected text and to-be-typed text at the caret
        function applyFormatToSelection(command, defaultValue, value) {
            const editor = document.getElementById('editor');
            editor.focus();
            ...
            // If there's a selection, just use execCommand directly
            if (selection.rangeCount > 0 && !selection.isCollapsed) {
                document.execCommand(command, false, value);
                return;
            }
            
            // If there's no selection, we need special handling for the caret position
            // Insert a zero-width space as a marker
            document.execCommand('insertHTML', false, '<span id="format-marker">&#8203;</span>');
            
            // Find our marker
            const marker = document.getElementById('format-marker');
            if (!marker) return;
            ...
            // Clean up - remove the marker but keep formatting
            ...
            if (parent) {
                ...
                // Position the caret at the right spot
                selection.removeAllRanges();
                const newRange = document.createRange();
                newRange.setStart(parent, Array.from(parent.childNodes).indexOf(marker));
