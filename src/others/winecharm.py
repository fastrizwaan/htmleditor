...
######################### CREATE BOTTLE
    ...
    def get_runner_for_create_bottle(self, script_data):
        """
        Extracts and resolves the runner for create_bottle.

        Args:
            script_data (dict): The script data containing runner information.

        Returns:
            str or Path: The resolved runner path or command.
        """
        runner = script_data.get('runner', '').strip()

        if not runner or runner in (str(Path.cwd()), str(Path.home())):
            if self.debug:
                print("Invalid runner detected, falling back to 'wine'.")
            runner = "wine"

        if self.debug:
            print(f"Using runner: {runner}")

        # Resolve path if it's not the default 'wine'
        if runner != "wine":
            try:
                runner = Path(runner).expanduser().resolve()
                if self.debug:
                    print(f"Runner resolved to: {runner}")
            except Exception as e:
                raise ValueError(f"Invalid runner path: {runner}. Error: {e}")
    ...
    def create_bottle_archive(self, script_key, wineprefix, backup_path):
        ...
        runner = self.expand_and_resolve_path(self.get_runner_for_create_bottle(script_data))
