...
<head>
    ...
    <style>
        ...
        .page {{
            ...
            height: {self.page_height_inches}in;
            padding: {self.page_margin_inches}in;
         ...
         .page > * {{ margin-bottom: 0.5em; margin-top: 0.5em; }}
...
<body contenteditable="true">
    <div class="page">
        {PLACEHOLDER_HTML} {{/* Python variable inserted correctly */}}
    </div>
    {{/* Additional pages added by JS */}}
</body>
</html>"""
        # --- End Initial HTML ---
        ...
        if load_event == WebKit.LoadEvent.FINISHED:
            pagination_js = f"""
                (function() {{
                    ...
                    const LINE_HEIGHT = 24; // Should match CSS line-height
                    ...
                    function maintainPlaceholder(page) {{
                        if (!page.textContent.trim() && !page.querySelector('.placeholder')) {{
                    ...
                    function paginateContent() {{
                        ...
                        while (currentPage.scrollHeight > PAGE_HEIGHT) {{
                            ...
                            // Move content until page is not overflowing
                            while (currentPage.scrollHeight > PAGE_HEIGHT) {{
                        ...
                        // Cleanup empty pages except last
                        document.querySelectorAll('.page').forEach((page, index, arr) => {{
                            if (index !== arr.length - 1 && page.textContent.trim() === '') {{
                                page.remove();
                            }}
                        }});
                    ...
                    function handleEnter(event) {{
                        if (event.key === 'Enter') {{
                            ...
                            const page = range.startContainer.closest('.page');
                            
                            // Estimate space after adding new line
                            const lineHeight = parseInt(getComputedStyle(page).lineHeight);
                            if (page.scrollHeight + lineHeight > PAGE_HEIGHT) {{
                                const newPage = createNewPage();
                    ...
                    function schedulePaginate() {{
                        ...
                        rafId = requestAnimationFrame(paginateContent);
                    }}

                    // Event listeners
                    document.addEventListener('input', schedulePaginate);
                    document.addEventListener('keydown', handleEnter);
                    document.addEventListener('paste', schedulePaginate);
