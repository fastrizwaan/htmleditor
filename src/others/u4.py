...
class PaginatedEditor(Adw.Application):
    def __init__(self):
        ...
        self.create_action('bold', self.on_bold_action)
        self.create_action('italic', self.on_italic_action)
        self.create_action('underline', self.on_underline_action)
    
    def create_action(self, name, callback):
        """Create a simple app action."""
        action = Gio.SimpleAction.new(name, None)
        action.connect("activate", callback)
        self.add_action(action)
        
    def on_bold_action(self, action, param):
        self.execute_editor_command('bold')
    
    def on_italic_action(self, action, param):
        self.execute_editor_command('italic')
    
    def on_underline_action(self, action, param):
        self.execute_editor_command('underline')
    
    def execute_editor_command(self, command):
        # Execute formatting command in WebView
        js_code = f"document.execCommand('{command}', false, null);"
        self.webview.evaluate_javascript(js_code, -1, None, None, None, None, None, None)
    ...
    def on_activate(self, app):
        ...
        # Create a popover menu model
        menu_model = Gio.Menu()
        menu_model.append("Bold", "app.bold")
        menu_model.append("Italic", "app.italic")
        menu_model.append("Underline", "app.underline")
        
        format_button.set_menu_model(menu_model)
    ...
    <script>
        ...
        class EditorController {
            ...
            getCleanHTMLContent() {
                ...
                // Clean replace divs with p tags and normalize content
...
"""
    ...
    def show_notification(self, message):
        ...
        # Print the message anyway for feedback
        print(message)
        
        try:
            # Try to get a toast overlay from the window's content
            ...
            if isinstance(content, Adw.ToastOverlay):
                ...
                # Create a new toast overlay and set it up
                toast_overlay = Adw.ToastOverlay()
                # Add the existing content to the toast overlay
                if content:
                    toast_overlay.set_child(content)
                # Set the toast overlay as the window's content
                self.win.set_content(toast_overlay)
            ...
            # If anything fails, just print the error
