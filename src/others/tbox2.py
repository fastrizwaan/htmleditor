...
<body>
    ...
    <script>
        ...
        function createTextBox(x, y) {
            ...
            // Add activation and border click handlers
            textBox.addEventListener('click', (e) => {
                activateTextBox(textBox);
                e.stopPropagation();
            });
            
            // Detect clicks on border to deactivate
            textBox.addEventListener('mousedown', (e) => {
                const rect = textBox.getBoundingClientRect();
                const borderSize = 5; // Increased border detection area
                const isTextArea = e.target.tagName === 'TEXTAREA';
                
                // Check if click is on the border area and not on the textarea
                if (!isTextArea && (
        ...
        // Deactivate the active text box
