...
.linked menubutton:first-child > button         {background-color: orange; border: solid 1px rgba(255, 27, 27, 0.30);border-top-left-radius: 5px; border-bottom-left-radius: 5px; border-top-right-radius: 0px; border-bottom-right-radius: 0px;}
.linked menubutton:last-child > button         {background-color: orange; border: solid 1px rgba(255, 27, 27, 0.30);border-top-left-radius: 0px; border-bottom-left-radius: 0px; border-top-right-radius: 5px; border-bottom-right-radius: 5px;}

.linked menubutton:active        {background-color: rgba(127, 127, 127, 0.35); border: solid 1px rgba(127, 127, 127, 0.00);}
.linked menubutton:checked       {background-color: rgba(127, 127, 127, 0.35); border: solid 1px rgba(127, 127, 127, 0.00);}
.linked menubutton:checked:hover {background-color: rgba(127, 127, 127, 0.35); border: solid 1px rgba(127, 127, 127, 0.30);}
"""
...
// Initialize any existing text boxes when the document loads
function initializeTextBoxes() {
    const textBoxes = document.querySelectorAll('.text-box');
    textBoxes.forEach(textBox => makeTextBoxResizable(textBox));
}"""
...
#####################
# Implementation for Text Box Insertion in HTML Editor

## 1. First, let's add a button to the file toolbar

# Locate the create_file_toolbar method and add a text box button to the spacing_group
# Add this after the column_button in the spacing_group:


    ## 2. Create the method to handle the text box insertion

    def on_insert_text_box_clicked(self, win, button):
        """Handle text box insertion button click"""
        self.show_text_box_dialog(win)
        
    def show_text_box_dialog(self, win):
        """Show dialog to configure text box properties"""
        dialog = Adw.Dialog()
        ...
        # Text box size options
        size_header = Gtk.Label()
        size_header.set_markup("<b>Text Box Size:</b>")
        size_header.set_halign(Gtk.Align.START)
        content_box.append(size_header)
        
        # Width and height selection
        size_grid = Gtk.Grid()
        size_grid.set_column_spacing(12)
        size_grid.set_row_spacing(12)
        size_grid.set_margin_top(12)
        
        # Width
        width_label = Gtk.Label(label="Width:")
        width_label.set_halign(Gtk.Align.START)
        
        width_adj = Gtk.Adjustment.new(200, 50, 600, 1, 10, 0)
        ...
        height_adj = Gtk.Adjustment.new(150, 50, 600, 1, 10, 0)
        ...
        content_box.append(size_grid)
        
        # Border and colors
        border_header = Gtk.Label()
        border_header.set_markup("<b>Border and Colors:</b>")
        border_header.set_halign(Gtk.Align.START)
        border_header.set_margin_top(12)
        ...
        border_adj = Gtk.Adjustment.new(1, 0, 10, 1, 1, 0)
        border_spin = Gtk.SpinButton()
        border_spin.set_adjustment(border_adj)
        ...
        wrap_header = Gtk.Label()
        wrap_header.set_markup("<b>Text Wrapping:</b>")
        wrap_header.set_halign(Gtk.Align.START)
        wrap_header.set_margin_top(12)
        content_box.append(wrap_header)
        
        # Wrapping options as radio buttons
        wrap_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=8)
        wrap_box.set_margin_top(12)
        
        # No wrap option
        no_wrap_radio = Gtk.CheckButton(label="No text wrapping")
        no_wrap_radio.set_active(False)
        wrap_box.append(no_wrap_radio)
        
        # Wrap left option
        wrap_left_radio = Gtk.CheckButton(label="Wrap text left of the box")
        wrap_left_radio.set_group(no_wrap_radio)
        wrap_box.append(wrap_left_radio)
        
        # Wrap right option
        wrap_right_radio = Gtk.CheckButton(label="Wrap text right of the box")
        wrap_right_radio.set_group(no_wrap_radio)
        wrap_right_radio.set_active(True)  # Default option
        wrap_box.append(wrap_right_radio)
        
        # Wrap both sides option
        wrap_both_radio = Gtk.CheckButton(label="Wrap text on both sides")
        wrap_both_radio.set_group(no_wrap_radio)
        wrap_box.append(wrap_both_radio)
        
        content_box.append(wrap_box)
        
        # Initial text
        text_header = Gtk.Label()
        text_header.set_markup("<b>Initial Text:</b>")
        text_header.set_halign(Gtk.Align.START)
        text_header.set_margin_top(12)
        content_box.append(text_header)
        
        # Text entry
        text_entry = Gtk.TextView()
        text_entry.set_wrap_mode(Gtk.WrapMode.WORD)
        text_entry.set_margin_top(8)
        text_entry.set_margin_bottom(8)
        text_entry.set_size_request(40,40)
        
        # Scroll window for text entry
        scroll_window = Gtk.ScrolledWindow()
        scroll_window.set_hexpand(True)
        scroll_window.set_vexpand(True)
        scroll_window.set_child(text_entry)
        
        content_box.append(scroll_window)
        ...
        insert_button.connect("clicked", lambda btn: self.insert_text_box(
            win, dialog, 
            width_spin.get_value_as_int(),
            height_spin.get_value_as_int(),
            border_spin.get_value_as_int(),
            text_entry.get_buffer().get_text(
                text_entry.get_buffer().get_start_iter(),
                text_entry.get_buffer().get_end_iter(),
                True
            ),
            "none" if no_wrap_radio.get_active() else
            "left" if wrap_left_radio.get_active() else
            "right" if wrap_right_radio.get_active() else
            "both"
        ))
    ...
    def insert_text_box(self, win, dialog, width, height, border_width, text, float_position):
        """Insert a text box with the specified properties"""
        float_style = ""
        margin_style = ""
        
        if float_position == "left":
            float_style = "float: left;"
            margin_style = "margin: 0 15px 15px 0;"
        elif float_position == "right":
            float_style = "float: right;"
            margin_style = "margin: 0 0 15px 15px;"
        elif float_position == "both":
            float_style = "display: block; margin: 0 auto;"
            margin_style = "margin: 15px auto;"
        
        # Prepare text content (escape for JS)
        import json
        text_json = json.dumps(text)
        
        # Create the HTML for the text box
        js_code = f"""
        (function() {{
            ...
            textBox.style.border = '{border_width}px solid #000';
            textBox.style.padding = '10px';
            textBox.style.boxSizing = 'border-box';
            textBox.style.backgroundColor = '#f9f9f9';
            textBox.style.overflow = 'auto';
            textBox.style.{float_style}
            textBox.style.{margin_style}
            textBox.innerHTML = {text_json};
            
            // Insert the text box at cursor position
            const selection = window.getSelection();
            if (selection.rangeCount > 0) {{
                const range = selection.getRangeAt(0);
                ...
                // Move cursor after the text box
                range.setStartAfter(textBox);
                range.setEndAfter(textBox);
                selection.removeAllRanges();
                selection.addRange(range);
                
                // Insert a line break to facilitate text wrapping
    ...
    ## 3. Add JavaScript code for text box handling to the editor initialization

    # Add this to the get_editor_html method, in the <style> section:
    """
    .text-box {
        min-width: 100px;
        min-height: 50px;
    ...
    # Add text box handling functions to the get_editor_js method:
    """
    // Text box handling functions
    function makeTextBoxResizable(textBox) {
        textBox.style.resize = 'both';
    ...
    # Add a call to initializeTextBoxes() in the init_editor_js() function:
    """
