...
##################################
    def tag_active_in_entire_selection(self, tag_name):
        bounds = self.buffer.get_selection_bounds()
        if not bounds:
            return False
    ...
    def any_tag_in_selection(self, tag_name):
        bounds = self.buffer.get_selection_bounds()
        if not bounds:
            return False
    ...
    def update_button_states(self):
        ...
        self.bold_button.set_active(self.tag_active_in_entire_selection("bold"))
        self.italic_button.set_active(self.tag_active_in_entire_selection("italic"))
        self.underline_button.set_active(self.tag_active_in_entire_selection("underline"))
...
#########################################
    def update_button_states(self):
        ...
        # Helper function to check tag presence
        def get_tag_state(tag_name):
            if bounds:
                tag = self.buffer.get_tag_table().lookup(tag_name)
                start, end = bounds
                has_tag = True
                ...
                return getattr(self.format_state, tag_name)

        # Update style buttons
        self.bold_button.set_active(get_tag_state("bold"))
        self.italic_button.set_active(get_tag_state("italic"))
        self.underline_button.set_active(get_tag_state("underline"))
        ...
        for align in ["left", "center", "right", "justify"]:
            ...
            if bounds:
                ...
                all_aligned = True
                while current.compare(end) < 0:
                    if not current.has_tag(align_tag):
                        all_aligned = False
                        break
                    if not current.forward_char():
                        break
                btn.set_active(all_aligned)
            else:
                btn.set_active(self.format_state.alignment == align)
    ...
    def on_bold_clicked(self, button):
        bounds = self.buffer.get_selection_bounds()
        if not bounds:
            # Toggle format state for new text
            self.format_state.bold = button.get_active()
            return

        tag = self.buffer.get_tag_table().lookup("bold")
        start, end = bounds
        
        # Check if entire selection is already bold
        all_bold = True
        current = start.copy()
        while current.compare(end) < 0:
            if not current.has_tag(tag):
        ...
        button.set_active(not all_bold)

    # Repeat identical logic for italic and underline
    def on_italic_clicked(self, button):
        bounds = self.buffer.get_selection_bounds()
        if not bounds:
            self.format_state.italic = button.get_active()
        ...
        tag = self.buffer.get_tag_table().lookup("italic")
        start, end = bounds
        ...
        button.set_active(not all_italic)
    ...
    def on_underline_clicked(self, button):
        ...
        tag = self.buffer.get_tag_table().lookup("underline")
        start, end = bounds
        ...
        button.set_active(not all_underline)
###################################
    def update_button_states(self):
        # Block signals to prevent recursion
        self.bold_button.handler_block(self.bold_handler_id)
        self.italic_button.handler_block(self.italic_handler_id)
        self.underline_button.handler_block(self.underline_handler_id)

        try:
            bounds = self.buffer.get_selection_bounds()
            cursor = self.buffer.get_iter_at_mark(self.buffer.get_insert())

            # Helper function to check tag presence in entire selection
            def get_tag_state(tag_name):
                if not bounds:
                    return getattr(self.format_state, tag_name)
            ...
            self.bold_button.handler_unblock(self.bold_handler_id)
            self.italic_button.handler_unblock(self.italic_handler_id)
            self.underline_button.handler_unblock(self.underline_handler_id)
    ...
    def on_italic_clicked(self, button):
        ...
        if bounds:
            start, end = bounds
            # Check if entire selection is already italic
            all_italic = True
            current = start.copy()
            while current.compare(end) < 0:
                if not current.has_tag(tag):
    ...
    def on_underline_clicked(self, button):
        ...
        if bounds:
            start, end = bounds
            # Check if entire selection is already underlined
            all_underline = True
            current = start.copy()
            while current.compare(end) < 0:
                if not current.has_tag(tag):
    ...
    def create_text_format_buttons(self, container):
        ...
        for name, icon, callback, tag in style_buttons_data:
            ...
            handler_id = button.connect("toggled", callback)
            setattr(self, f"{tag}_handler_id", handler_id)
            button.add_css_class("flat")
            setattr(self, f"{tag}_button", button)
            container.append(button) 
######################
        ...
        try:
            ...
            if text:
                ...
                if "<html>" in text.lower() or "<body>" in text.lower():
                    self.paste_html_content(text)
                else:
                    self.buffer.insert_at_cursor(text, -1)
        except GLib.Error as error:
            print(f"Error pasting: {error.message}")

    def paste_html_content(self, html):
        ...
        class HTMLFilter(HTMLParser):
            ...
            def handle_endtag(self, tag):
                if self.current_tags and self.current_tags[-1] == tag.lower():
        ...
        # Insert the filtered text with basic formatting
        self.buffer.insert_at_cursor(parser.text, -1)
############################
