...
class HTMLEditorApp(Adw.Application):
    ...
    def create_window(self):
        ...
        self.setup_size_monitoring(win)
        
        # Initial size check after a short delay to ensure window is fully loaded
        GLib.timeout_add(300, lambda: self.check_window_size(win))
        
        return win

        
# Add this to the HTMLEditorApp class

# Add this to the HTMLEditorApp class

    def setup_headerbar_content(self, win):
        """Create buttons for the header bar with responsive design"""
        ...
        win.header_buttons['new'] = new_button
        ...
        win.header_buttons['open'] = open_button
        ...
        win.header_buttons['save'] = save_button
        ...
        win.header_buttons['undo'] = win.undo_button
        ...
        win.header_buttons['redo'] = win.redo_button
        ...
        win.header_buttons['menu'] = menu_button
        
        # Create overflow menu for small window sizes
        win.overflow_button = Gtk.MenuButton()
        win.overflow_button.set_icon_name("view-more-symbolic")
        win.overflow_button.set_tooltip_text("More Actions")
        
        # Create the overflow menu model (will be populated dynamically)
        win.overflow_menu = Gio.Menu()
        
        # Initially add all actions to the overflow menu
        file_section = Gio.Menu()
        file_section.append("New Document", "app.new-document")
        file_section.append("Open File", "app.open-file")
        file_section.append("Save", "app.save-file")
        
        edit_section = Gio.Menu()
        edit_section.append("Undo", "app.undo")
        edit_section.append("Redo", "app.redo")
        
        win.overflow_menu.append_section("File", file_section)
        win.overflow_menu.append_section("Edit", edit_section)
        
        win.overflow_button.set_menu_model(win.overflow_menu)
        win.overflow_button.set_visible(False)  # Initially hidden
        win.header_buttons['overflow'] = win.overflow_button
        ...
        win.headerbar.pack_end(win.overflow_button)
        
        # Create window menu button (for window management)
        self.add_window_menu_button(win)
        
        # Setup size monitoring using layout manager
        win.layout_manager = Gtk.LayoutManager.new()
        win.layout_manager.connect("layout-changed", lambda manager: self.check_window_size(win))
        
        # Initial check of window size
        GLib.timeout_add(100, lambda: self.check_window_size(win))

    def check_window_size(self, win):
        ...
        # Implement responsive behavior based on width
        if width < narrow_width:
            # Very narrow: Show only overflow and main menu
            self.set_button_visibility(win, {
                'new': False,
                'open': False,
                'save': False,
                'undo': False,
            ...
            self.update_overflow_menu(win, ['new', 'open', 'save', 'undo', 'redo'])
        elif width < medium_width:
            # Medium width: Show essential buttons
            self.set_button_visibility(win, {
                'new': False,
                'open': True,
                'save': True,
            ...
            self.update_overflow_menu(win, ['new', 'undo', 'redo'])
        else:
        ...
        # Continue monitoring if this was called from a timeout
        return False

    def set_button_visibility(self, win, visibility_map):
        """Set the visibility of header bar buttons"""
        for button_id, visible in visibility_map.items():
            if button_id in win.header_buttons:
                win.header_buttons[button_id].set_visible(visible)

    def update_overflow_menu(self, win, button_ids):
        """Update the overflow menu with appropriate actions"""
        # Create actions for each button that should be in overflow menu
        if not hasattr(win, 'overflow_actions'):
            win.overflow_actions = {}
        
        # Clear existing actions if there are any
        for action_name in win.overflow_actions:
            if win.has_action(action_name):
                win.remove_action(action_name)
        
        win.overflow_actions = {}
        
        # Create new actions
        for button_id in button_ids:
            action_name = f"overflow-{button_id}"
            action = Gio.SimpleAction.new(action_name, None)
            
            if button_id == 'new':
                action.connect("activate", lambda a, p: self.on_new_clicked(win, None))
            elif button_id == 'open':
                action.connect("activate", lambda a, p: self.on_open_clicked(win, None))
            elif button_id == 'save':
                action.connect("activate", lambda a, p: self.on_save_clicked(win, None))
            elif button_id == 'undo':
                action.connect("activate", lambda a, p: self.on_undo_clicked(win, None))
            elif button_id == 'redo':
                action.connect("activate", lambda a, p: self.on_redo_clicked(win, None))
            
            win.add_action(action)
            win.overflow_actions[action_name] = action
        
        # Rebuild the overflow menu
        file_actions = []
        edit_actions = []
        
        if 'new' in button_ids:
            file_actions.append(("New Document", "win.overflow-new"))
        if 'open' in button_ids:
            file_actions.append(("Open File", "win.overflow-open"))
        if 'save' in button_ids:
            file_actions.append(("Save", "win.overflow-save"))
        if 'undo' in button_ids:
            edit_actions.append(("Undo", "win.overflow-undo"))
        if 'redo' in button_ids:
            edit_actions.append(("Redo", "win.overflow-redo"))
        
        # Create new menu model
        new_overflow_menu = Gio.Menu()
        
        if file_actions:
            file_section = Gio.Menu()
            for label, action in file_actions:
                file_section.append(label, action)
            new_overflow_menu.append_section("File", file_section)
        
        if edit_actions:
            edit_section = Gio.Menu()
            for label, action in edit_actions:
                edit_section.append(label, action)
            new_overflow_menu.append_section("Edit", edit_section)
        
        # Update menu button model
        win.overflow_button.set_menu_model(new_overflow_menu)

    # Add this to HTMLEditorApp for size monitoring
    def setup_size_monitoring(self, win):
        """Set up window size monitoring using Gtk.EventControllerMotion"""
        # Create a controller for motion events
        controller = Gtk.EventControllerMotion.new()
        controller.connect("motion", lambda c, x, y: self.on_window_motion(win))
        win.add_controller(controller)
        
        # Also monitor window state events for maximize/unmaximize
        win.connect("notify::maximized", lambda obj, pspec: self.check_window_size(win))
        
        # Store the controller
        win.size_monitor_controller = controller
        
        # Set up a timer to periodically check window size
        win.size_check_timer = GLib.timeout_add(500, lambda: self.periodic_size_check(win))

    def on_window_motion(self, win):
        """Trigger a size check when there's mouse movement (can detect resize)"""
        # Only check occasionally to avoid too many updates
        if not hasattr(win, 'last_size_check') or time.time() - win.last_size_check > 0.2:
            win.last_size_check = time.time()
            self.check_window_size(win)
        return False

    def periodic_size_check(self, win):
        """Periodically check window size"""
        if win.is_visible():
            self.check_window_size(win)
        return True  # Continue the timer
    def on_window_size_changed(self, win, allocation):
        """Handle window size changes to implement responsive design"""
        # Get current window width
        width = win.get_width()
...
# Update your on_window_close_request method to clean up resources

    def on_window_close_request(self, win, *args):
        """Handle window close request with save confirmation if needed"""
        # Clean up monitoring resources
        if hasattr(win, 'size_check_timer') and win.size_check_timer > 0:
            GLib.source_remove(win.size_check_timer)
            win.size_check_timer = 0
    ...
    def show_error_dialog(self, message):
        ...
        dialog.present(active_window)
#
# New methods to implement responsive design in HTMLEditorApp
