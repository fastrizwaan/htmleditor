...
class HTMLEditorApp(Adw.Application):
    ...
    def create_window(self):
        ...
        # Create a proper container for the toolbar first
        toolbar_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL)
        
        # Create top toolbar (below header)
        win.top_toolbar = self.create_top_toolbar(win)
        
        # Add toolbar to its container
        toolbar_box.append(win.top_toolbar)
        
        # Create revealer and add the toolbar container to it
        win.toolbar_revealer = Gtk.Revealer()
        win.toolbar_revealer.set_transition_type(Gtk.RevealerTransitionType.SLIDE_DOWN)
        win.toolbar_revealer.set_transition_duration(300)  # 300ms animation
        win.toolbar_revealer.set_child(toolbar_box)
        win.toolbar_revealer.set_reveal_child(True)  # Initially visible
        
        # Add revealer to main box
        win.main_box.append(win.toolbar_revealer)
    ...
    def toggle_top_toolbar(self, win, *args):
        """Toggle the visibility of the top toolbar with smooth animation"""
        if hasattr(win, 'toolbar_revealer'):
            win.top_toolbar_visible = not win.top_toolbar_visible
            win.toolbar_revealer.set_reveal_child(win.top_toolbar_visible)
            
            win.statusbar.set_text(f"Formatting toolbar {'shown' if win.top_toolbar_visible else 'hidden'}")
            return True
        else:
            print("Window does not have toolbar_revealer attribute")
        return False       
