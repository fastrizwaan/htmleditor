        ...
        (function() {
            ...
            function handleDrag(e) {
                ...
                if (range) {
                    ...
                    // Preserve the alignment and wrapping classes
                    const classes = selectedImage.className;
                    const styles = {
                        float: selectedImage.style.float,
                        margin: selectedImage.style.margin,
                        display: selectedImage.style.display,
                        shapeOutside: selectedImage.style.shapeOutside
                    };
                    
                    temp.parentNode.insertBefore(selectedImage, temp);
                    
                    // Reapply the saved styles
                    selectedImage.className = classes;
                    Object.keys(styles).forEach(style => {
                        if (styles[style]) {
                            selectedImage.style[style] = styles[style];
                        }
                    });
                    
                    temp.remove();
                    
                    // Check if we need to reapply text wrapping based on new position
                    if (selectedImage.classList.contains('align-left') || 
                        selectedImage.classList.contains('align-right')) {
                        // Get current wrap type from active style
                        let wrapType = 'around';
                        if (selectedImage.style.margin === '0 8px 5px 0' || 
                            selectedImage.style.margin === '0 0 5px 8px') {
                            wrapType = 'tight';
                        } else if (selectedImage.style.shapeOutside === 'margin-box') {
                            wrapType = 'square';
                        }
                        
                        // Reapply the wrap based on new position
                        setTextWrap(selectedImage, wrapType);
                    }
            ...
            function createContextMenu(x, y) {
                ...
                const menuItems = [
                    ...
                    { label: 'Text Wrap', submenu: [
                        ...
                        { label: 'Tight', action: 'wrap-tight' },
                        { label: 'Square', action: 'wrap-square' },
            ...
            // Helper function to detect if browser supports shape-outside
            function supportsShapeOutside() {
                const testElement = document.createElement('div');
                return 'shapeOutside' in testElement.style || 
                       'webkitShapeOutside' in testElement.style;
            }
            ...
            function handleContextMenuAction(action) {
                ...
                switch (action) {
                    ...
                    case 'wrap-around':
                        ...
                        break;
                    case 'wrap-tight':
                        setTextWrap(selectedImage, 'tight');
                        break;
                    case 'wrap-square':
                        setTextWrap(selectedImage, 'square');
                        break;
                    case 'wrap-none':
            ...
            function setImageAlignment(image, alignment) {
                // Remove existing alignment and wrap classes
                ...
                switch (alignment) {
                    case 'left':
                        ...
                        image.style.shapeOutside = 'margin-box';
                        break;
                    case 'center':
                    ...
                    case 'right':
                        ...
                        break;
                    case 'none':
                        image.classList.add('align-none');
                        image.style.float = 'none';
            ...
            function setTextWrap(image, wrap) {
                // Remove any previous wrapping styles
                image.classList.remove('align-left', 'align-right', 'align-center', 'align-none');
                image.style.float = '';
                image.style.margin = '';
                
                // Clear any previous text-wrap-none elements
                const nextSibling = image.nextSibling;
                if (nextSibling && nextSibling.classList && nextSibling.classList.contains('text-wrap-none')) {
                    nextSibling.remove();
                }
                ...
                if (wrap === 'none') {
                    // No wrapping - text comes after the image
                    image.classList.add('align-none');
                    image.style.display = 'block';
                    const clearDiv = document.createElement('div');
                    clearDiv.className = 'text-wrap-none';
                    ...
                    // Text wraps around the image based on its position
                    // Determine best alignment based on the image's position in the editor
                    const editorRect = editor.getBoundingClientRect();
                    const imageRect = image.getBoundingClientRect();
                    const imageCenter = imageRect.left + (imageRect.width / 2);
                    const editorCenter = editorRect.left + (editorRect.width / 2);
                    
                    if (imageCenter < editorCenter) {
                        // Image is on the left side, align left
                        image.classList.add('align-left');
                        image.style.float = 'left';
                        image.style.margin = '0 15px 10px 0';
                    } else {
                        // Image is on the right side, align right
                        image.classList.add('align-right');
                        image.style.float = 'right';
                        image.style.margin = '0 0 10px 15px';
                    }
                } else if (wrap === 'tight') {
                    // Tight wrapping (less margin)
                    // Similar to 'around' but with less margin
                    const editorRect = editor.getBoundingClientRect();
                    const imageRect = image.getBoundingClientRect();
                    const imageCenter = imageRect.left + (imageRect.width / 2);
                    const editorCenter = editorRect.left + (editorRect.width / 2);
                    ...
                    // Square wrapping (text follows the square shape of the image)
                    const editorRect = editor.getBoundingClientRect();
                    const imageRect = image.getBoundingClientRect();
                    const imageCenter = imageRect.left + (imageRect.width / 2);
                    const editorCenter = editorRect.left + (editorRect.width / 2);
