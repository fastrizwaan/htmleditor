def setup_text_tags(self):
    ...
    def on_line_spacing_changed(self, dropdown, param_spec):
        ...
        if self.textbuffer.get_has_selection():
            ...
            # Apply new spacing
            self.textbuffer.apply_tag_by_name(f"spacing-{spacing}", para_start, para_end)#!/usr/bin/env python3
import sys
import os
import gi
...
class RichTextEditor(Adw.Application):
    ...
    def on_activate(self, app):
        ...
        self.font_combo = Gtk.DropDown()
        font_model = Gio.ListStore.new(Gtk.StringObject)
        
        for font_family in font_families:
            font_model.append(Gtk.StringObject.new(font_family.get_name()))
        
        self.font_combo.set_model(font_model)
        self.font_combo.set_selected(0)  # Default to first font
        self.font_combo.set_hexpand(True)
        self.font_combo.set_margin_end(10)
        
        # Set up the factory for displaying the font names
        factory = Gtk.SignalListItemFactory()
        factory.connect("setup", self._on_factory_setup)
        factory.connect("bind", self._on_factory_bind)
        self.font_combo.set_factory(factory)
        ...
        self.font_size_combo = Gtk.DropDown()
        ...
        self.font_size_combo.set_model(size_model)
        # Set default size to 12pt (index 4)
        self.font_size_combo.set_selected(4)
        ...
        self.font_size_combo.set_factory(size_factory)
        
        self.font_size_combo.connect("notify::selected", self.on_font_size_selected)
        ...
        para_label = Gtk.Label.new("Paragraph Style:")
        para_label.set_margin_start(5)
        self.para_box.append(para_label)
        
        # Paragraph style combo box
        self.para_combo = Gtk.DropDown()
        ...
        self.para_combo.set_model(para_model)
        self.para_combo.set_selected(0)  # Default to normal
        ...
        self.para_combo.set_factory(para_factory)
        ...
        self.line_spacing_combo = Gtk.DropDown()
        ...
        self.line_spacing_combo.set_model(spacing_model)
        self.line_spacing_combo.set_selected(0)  # Default to 1.0
        ...
        self.line_spacing_combo.set_factory(spacing_factory)
        
        self.line_spacing_combo.connect("notify::selected", self.on_line_spacing_changed)
        ...
        self.current_font_family = font_families[0].get_name()
        # Default font size
    ...
    def on_color_set(self, button):
        ...
        if self.textbuffer.get_has_selection():
            ...
            if bounds:
                ...
                # Get all tags at the start of the selection
                current_tags = []
    ...
    def generate_html(self):
        ...
        while offset < len(text):
            ...
            if tags or offset == 0:
                ...
                if para_style and not self.current_para_style:
                    ...
                    if para_style in ["h1", "h2", "h3"]:
                        ...
                        self.in_paragraph = False  # Headings are their own block elements
                        self.current_para_style = para_style
                        ...
                        self.in_paragraph = True
                        self.current_para_style = para_style
                elif alignment and not self.in_paragraph:
                    style_attr = f"text-align: {alignment};"
                    if line_spacing:
                        style_attr += f" line-height: {line_spacing};"
            ...
            if offset < len(text) and text[offset] == '\n':
                ...
                if self.in_paragraph:
                    html_parts.append("</p>")
                    self.in_paragraph = False
                elif self.current_para_style in ["h1", "h2", "h3"]:
                    html_parts.append(f"</{self.current_para_style}>")
                    self.current_para_style = None
                elif self.current_para_style == "blockquote":
                    html_parts.append("</blockquote>")
                    self.current_para_style = None
                elif self.current_para_style == "pre":
                    html_parts.append("</pre>")
                    self.current_para_style = None
                
                # Add line break in regular text
                if not self.current_para_style:
                ...
                if next_iter.get_char():  # Make sure we're not at end of buffer
                    ...
                    for tag in next_tags:
                        ...
                        if tag_name == "heading-1":
                            ...
                            self.current_para_style = "h1"
                            need_paragraph = False
                        elif tag_name == "heading-2":
                            html_parts.append("<h2>")
                            self.current_para_style = "h2" 
                            need_paragraph = False
                        elif tag_name == "heading-3":
                            html_parts.append("<h3>")
                            self.current_para_style = "h3"
                            need_paragraph = False
                        elif tag_name == "blockquote":
                            style_attr = "margin-left: 30px; margin-right: 30px; font-style: italic;"
                            if next_alignment:
                            ...
                            html_parts.append(f'<blockquote style="{style_attr}">')
                            self.current_para_style = "blockquote"
                            need_paragraph = False
                            self.in_paragraph = True
                        elif tag_name == "preformatted":
                            html_parts.append('<pre style="margin-left: 20px; font-family: monospace;">')
                            self.current_para_style = "pre"
                            need_paragraph = False
                            self.in_paragraph = True
                        elif tag_name in ["left", "center", "right"]:
                ...
                if need_paragraph:
                    ...
                    if self.paragraph_align:
                        style_attr += f"text-align: {self.paragraph_align};"
