...
<head>
    <style>
        ...
        @media (prefers-color-scheme: dark) {
            ...
            th.selected, td.selected { background-color: rgba(94, 151, 246, 0.25); outline-color: #5e97f6; }
        }
        @media (prefers-color-scheme: light) {
            ...
            th.selected, td.selected { background-color: rgba(66, 133, 244, 0.2); outline-color: #4285f4; }
        }
        #editor {
            outline: none;
            margin: 0px;
...
############################################
    ...
    def insert_table(self, rows, cols, has_header=True, border_width=1, border_style="solid"):
        border_style_attr = f"border: {border_width}px {border_style} black;"
        table_html = f'<div class="table-wrapper" style="display: inline-block; margin: 10px 0; {border_style_attr}"><table style="border-collapse: collapse; width: 100%; {border_style_attr}">'
        
        for row in range(rows):
            table_html += '<tr>'
            for col in range(cols):
                cell_tag = 'th' if row == 0 and has_header else 'td'
                table_html += f'<{cell_tag} style="{border_style_attr} padding: 8px; min-width: 50px;">&nbsp;</{cell_tag}>'
            table_html += '</tr>'
        
        table_html += '</table></div><p></p>'
        self.exec_js(f'document.execCommand("insertHTML", false, {json.dumps(table_html)});')
        self.setup_table_handlers()
    ...
    def setup_table_handlers(self):
        ...
        (function() {
            ...
            function setupCellEvents(cell) {
                cell.tabIndex = 0;  // Make focusable and in tab order
                const newCell = cell.cloneNode(true);
                cell.parentNode.replaceChild(newCell, cell);
                cell = newCell;
                cell.addEventListener('keydown', handleCellKeyDown);
                    ...
                    switch (e.key) {
                        ...
                        case 'ArrowDown':
                            if (rowIndex < rows.length - 1) {
                                ...
                                nextCell = newRow.cells[colIndex];
                            }
                            break;
                        case 'ArrowLeft':
                            if (colIndex > 0) {
            ...
            function createTableContextMenu(x, y) {
                ...
                const menuItems = [
                    ...
                    { label: 'Table Properties', submenu: [
                        ...
                        { label: 'Align Center', action: 'table-align-center' },
                        { label: 'Align Right', action: 'table-align-right' },
                        { label: 'Inline with Text', action: 'table-inline' },
                        { label: 'Make Floating', action: 'table-floating' }
                    ]},
                    { label: 'Cell Properties', submenu: [
                        { label: 'Set as Header', action: 'set-as-header' },
            ...
            function handleTableContextMenuAction(action) {
                ...
                switch (action) {
                    ...
                    case 'table-align-right':
                        ...
                        break;
                    case 'table-inline':
                        setTableAlignment(table, 'inline');
                        break;
                    case 'table-floating':
                        setTableFloating(table);
                        break;
            ...
            function setTableAlignment(table, alignment) {
                const wrapper = table.closest('.table-wrapper');
                if (!wrapper) return;
                wrapper.classList.remove('floating', 'align-left', 'align-center', 'align-right', 'inline');
                wrapper.style.position = '';
                wrapper.style.left = '';
                wrapper.style.top = '';
                wrapper.style.zIndex = '';
            ...
            function setTableFloating(table) {
                ...
                if (!wrapper) return;
                wrapper.classList.add('floating');
                wrapper.classList.remove('align-left', 'align-center', 'align-right', 'inline');
                wrapper.style.position = 'absolute';
                if (!wrapper.style.left || !wrapper.style.top) {
                    const rect = wrapper.getBoundingClientRect();
                    const editorRect = editor.getBoundingClientRect();
            ...
            function startColumnResize(e) {
                ...
                for (let i = 0; i < currentTable.rows.length; i++) {
                    ...
                    if (columnIndex < row.cells.length) {
                        const cell = row.cells[columnIndex];
                        cells.push({ element: cell, width: cell.offsetWidth });
                    }
                }
...
############################################        
    def setup_table_handlers(self):
        ...
        (function() {
            ...
            // Set up click handler outside tables to clear selection
            editor.addEventListener('click', (e) => {
                // Only clear if clicking directly on the editor (not on a table or inside a table)
                if (e.target === editor) {
                    clearTableSelection();
                }
            });
            ...
            function clearTableSelection() {
                ...
                // Keep the currentTable reference for potential later use
            }
            ...
            function setupTableEvents(table) {
                ...
                // Stop propagation to prevent editor's click handler from clearing selection
            ...
            function setupCellEvents(cell) {
                ...
                // Stop propagation to prevent selection clearing
            ...
            function selectCell(cell) {
                ...
                // Add selection tracking but without visible highlight
                cell.classList.add('selected');
                currentCell = cell;
                currentTable = cell.closest('table');
            ...
            function createTableContextMenu(x, y) {
                ...
                const menuItems = [
                    ...
                    { label: 'Split Cell', action: 'split-cell' },
                    { type: 'separator' },
                    { label: 'Table Borders', submenu: [
                        { label: 'Solid', action: 'border-style-solid' },
                        { label: 'Dotted', action: 'border-style-dotted' },
                        { label: 'Dashed', action: 'border-style-dashed' },
                        { label: 'Double', action: 'border-style-double' },
                        { label: 'None', action: 'border-style-none' }
                    ]},
                    { label: 'Table Position', submenu: [
                        { label: 'Align Left', action: 'table-align-left' },
                        { label: 'Align Center', action: 'table-align-center' },
                        { label: 'Align Right', action: 'table-align-right' },
                        { label: 'Float Left', action: 'table-float-left' },
                        { label: 'Float Right', action: 'table-float-right' },
                        { label: 'Make Floating (Movable)', action: 'table-floating' }
                    ]},
                    { label: 'Cell Properties', submenu: [
                        { label: 'Set as Header', action: 'set-as-header' },
                        { label: 'Set as Data Cell', action: 'set-as-data' },
            ...
            function handleTableContextMenuAction(action) {
                ...
                switch (action) {
                    ...
                    case 'border-style-solid':
                        setTableBorderStyle(table, 'solid');
                        break;
                    case 'border-style-dotted':
                        setTableBorderStyle(table, 'dotted');
                        break;
                    case 'border-style-dashed':
                        setTableBorderStyle(table, 'dashed');
                        break;
                    case 'border-style-double':
                        setTableBorderStyle(table, 'double');
                        break;
                    case 'border-style-none':
                        setTableBorderStyle(table, 'none');
                        break;
                    ...
                    case 'table-align-right':
                        setTableAlignment(table, 'right');
                        break;
                    case 'table-float-left':
                        setTableFloat(table, 'left');
                        break;
                    case 'table-float-right':
                        setTableFloat(table, 'right');
                        break;
                    case 'table-floating':
                        setTableFloating(table);
                        break;
            ...
            function setTableBorderStyle(table, style) {
                if (!table) return;
                
                // Get current border width
                const computedStyle = window.getComputedStyle(table);
                const borderWidth = computedStyle.borderWidth || '1px';
                const borderColor = computedStyle.borderColor || 'black';
                
                // Apply style to table and all cells
                if (style === 'none') {
                    ...
                    table.style.border = `${borderWidth} ${style} ${borderColor}`;
                    
                    // Update all cells with the same style
                    table.querySelectorAll('th, td').forEach(cell => {
                        const cellComputedStyle = window.getComputedStyle(cell);
                        const cellBorderWidth = cellComputedStyle.borderWidth || borderWidth;
                        const cellBorderColor = cellComputedStyle.borderColor || borderColor;
                        
                        cell.style.border = `${cellBorderWidth} ${style} ${cellBorderColor}`;
                    });
                }
            }
            ...
            function setTableAlignment(table, alignment) {
                ...
                // Clear any alignment classes
                wrapper.classList.remove('align-left', 'align-center', 'align-right', 'float-left', 'float-right');
            ...
            // Set table float
            // Set table float
            function setTableFloat(table, direction) {
                const wrapper = table.closest('.table-wrapper');
                if (!wrapper) return;
                
                // Remove floating position if it was absolute
                wrapper.classList.remove('floating');
                wrapper.style.position = '';
                wrapper.style.left = '';
                wrapper.style.top = '';
            ...
            function startTableMove(e, table, wrapper) {
                ...
                function stopTableMove() {
                    ...
                    // If the table is not set as floating, restore center alignment
                    if (!wrapper.classList.contains('floating')) {
                        setTableAlignment(table, 'center');
                    }
                }
            ...
            function insertRow(table, rowIndex) {
                ...
                for (let i = 0; i < refRow.cells.length; i++) {
                    ...
                    // Copy border styles
                    const computedStyle = window.getComputedStyle(refCell);
                    newCell.style.borderStyle = computedStyle.borderStyle;
                    newCell.style.borderWidth = computedStyle.borderWidth;
                    newCell.style.borderColor = computedStyle.borderColor;
                }
            ...
            function insertColumn(table, colIndex) {
                for (let i = 0; i < table.rows.length; i++) {
                    ...
                    if (refCell.tagName === 'TH') {
                        ...
                        // Copy border styles
                        const computedStyle = window.getComputedStyle(refCell);
                        th.style.borderStyle = computedStyle.borderStyle;
                        th.style.borderWidth = computedStyle.borderWidth;
                        th.style.borderColor = computedStyle.borderColor;
                        ...
                        // Copy border styles
                        const computedStyle = window.getComputedStyle(refCell);
                        td.style.borderStyle = computedStyle.borderStyle;
                        td.style.borderWidth = computedStyle.borderWidth;
                        td.style.borderColor = computedStyle.borderColor;
            ...
            tableStyles.textContent = `
                ...
                /* Remove visible selection styles */
                th.selected, td.selected {
