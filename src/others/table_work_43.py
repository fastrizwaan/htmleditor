                ...
                if (hasHeader) {
                    ...
                    for (let i = 0; i < cols; i++) {
                        if (i === Math.floor(cols/2)) {
                            tableHtml += '<th contenteditable="true" style="position: relative;"><div class="drag-handle" style="position: absolute; top: 0; left: 50%; transform: translateX(-50%); width: 50px; height: 5px; cursor: move;" contenteditable="false"></div>Header ' + (i+1) + '</th>';
                        } else {
                            tableHtml += '<th contenteditable="true">Header ' + (i+1) + '</th>';
                        }
                    }
                ...
                for (let r = 0; r < rows; r++) {
                    ...
                    for (let c = 0; c < cols; c++) {
                        if (r === 0 && !hasHeader && c === Math.floor(cols/2)) {
                            tableHtml += '<td contenteditable="true" style="position: relative;"><div class="drag-handle" style="position: absolute; top: 0; left: 50%; transform: translateX(-50%); width: 50px; height: 5px; cursor: move;" contenteditable="false"></div><br></td>';
                        } else if (r === rows - 1 && c === cols - 1) {
                            tableHtml += '<td contenteditable="true" style="position: relative;"><br><div class="resize-handle" style="position: absolute; right: 0; bottom: 0; width: 0; height: 0; border-right: 10px solid transparent; border-top: 10px solid transparent; cursor: se-resize;" contenteditable="false"></div></td>';
                        } else {
                            tableHtml += '<td contenteditable="true"><br></td>';
                        }
                    }
