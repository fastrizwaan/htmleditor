...
<body><p></p></body>
    ...
    def on_font_size_changed(self, dropdown, *args):
        if item := dropdown.get_selected_item():
            ...
            script = f"document.execCommand('fontSize', false, '7'); " \
                    f"document.querySelectorAll('*').forEach(el => " \
                    f"{{ if (window.getComputedStyle(el).fontSize === 'xx-large') el.style.fontSize = '{size_pt}pt'; }});"
            self.exec_js(script)
            self.update_formatting_ui()
