    ...
    def on_webview_load(self, webview, load_event):
        if load_event == WebKit.LoadEvent.FINISHED:
            ...
            (function() {
                ...
                // Add custom CSS for alignment and text wrapping
                ...
                style.textContent = `
                    ...
                    .resizing { outline: 2px dashed #007bff; }
                    .dragging {
                ...
                // Handle resize with aspect ratio preservation for images
                ...
                function startDrag(e, element) {
                    ...
                    // Store original position and float values
                    originalPosition = element.style.position;
                    originalFloat = element.style.float;
                    ...
                    // Only set absolute positioning when dragging starts
                    element.classList.add('dragging');
                    element.style.left = (rect.left - editorRect.left + editor.scrollLeft) + 'px';
                    element.style.top = (rect.top - editorRect.top + editor.scrollTop) + 'px';
                ...
                function handleDrag(e) {
                    ...
                    const maxLeft = editor.clientWidth - selectedElement.offsetWidth;
                    const maxTop = editor.clientHeight - selectedElement.offsetHeight;
                    
                    selectedElement.style.left = Math.max(0, Math.min(newLeft, maxLeft)) + 'px';
                    selectedElement.style.top = Math.max(0, Math.min(newTop, maxTop)) + 'px';
                ...
                function stopDrag() {
                    ...
                    // Restore original position and float if not dragging
                    if (!isDragging) {
                        selectedElement.classList.remove('dragging');
                        if (originalPosition === '') {
                            selectedElement.style.removeProperty('position');
                        } else {
                            selectedElement.style.position = originalPosition;
                        }
                        
                        if (originalFloat === '') {
                            selectedElement.style.removeProperty('float');
                        } else {
                            selectedElement.style.float = originalFloat;
                        }
                ...
                // Create context menu with border styles
                ...
                function setElementAlignment(element, alignment) {
                    // Remove all alignment classes and reset positioning
                    ...
                    // Apply styling based on alignment
                    if (alignment === 'center') {
                ...
                function setTextWrap(element, wrap) {
                    ...
                    if (wrap === 'none') {
                        element.classList.add('align-none');
                ...
                editor.querySelectorAll('img, .textbox').forEach(el => {
                    ...
                    if (el.classList.contains('textbox') && !el.style.border) {
                        ...
                        mutation.addedNodes.forEach(node => {
                            if (node.nodeType === 1) {
                                if (isSelectableElement(node)) {
                                    ...
                                    if (node.classList.contains('textbox') && !node.style.border) {
                ...
                window.insertAndSelectImage = function(src) {
                    ...
                    selectElement(img);
                    setElementAlignment(img, 'left');
                    setTextWrap(img, 'around');
                };

                // Insert and select textbox
                window.insertAndSelectTextbox = function() {
                    ...
                    handleTextboxEvents(textbox);
                    selectElement(textbox);
                    setElementAlignment(textbox, 'left');
                    setTextWrap(textbox, 'around');
                };
            })();
            """
