...
<body>
    ...
    def setup_image_handling(self):
        ...
        (function() {
            ...
            function setBorder(image, width, color) {
                if (!image) return;
                ...
                if (width === '0') {
                    ...
                    image.style.border = `${width}px solid ${currentBorderColor}`;
                }
            }
            ...
            function selectImage(image) {
                ...
                currentBorderWidth = image.style.borderWidth ? image.style.borderWidth.replace('px', '') : '0';
                currentBorderColor = image.style.borderColor || '#000000';
                currentBackgroundColor = image.style.backgroundColor || 'transparent';
            ...
            function createContextMenu(x, y) {
                ...
                const menuItems = [
                    ...
                    { label: 'Border', submenu: [
                        { label: 'None', action: 'border-0' },
                        { label: 'Thin (1px)', action: 'border-1' },
                        { label: 'Medium (3px)', action: 'border-3' },
                        { label: 'Thick (5px)', action: 'border-5' },
                        { type: 'separator' },
            ...
            function createMenuItems(parent, items) {
                items.forEach(item => {
                    if (item.type === 'separator') {
                        ...
                        if (item.action.startsWith('border-') && !item.action.includes('color')) {
                            const width = item.action.replace('border-', '');
            ...
            function handleContextMenuAction(action) {
                ...
                } else if (action.startsWith('border-')) {
                    if (action === 'border-color') {
                        createColorPickerDialog('Choose Border Color', currentBorderColor, (color) => {
                            setBorder(selectedImage, currentBorderWidth, color);
                        });
                    } else {
                        const width = action.replace('border-', '');
                        setBorder(selectedImage, width);
                    }
                ...
                try {
                    ...
                    canvas.toBlob(blob => {
                        try {
                            ...
                            // Fallback method: create a temporary link with download attribute
                            ...
                            setTimeout(() => {
                                ...
                                alert('Image could not be copied to clipboard. It has been downloaded instead.');
                            }, 0);
                        }
                    });
                } catch (err) {
                    console.error('Error creating canvas from image:', err);
