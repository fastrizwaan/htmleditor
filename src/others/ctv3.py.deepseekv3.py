...
class RichTextEditor(Adw.Application):
    def __init__(self):
        ...
        self.pending_tags = set()  # To store tags to be applied to new text
    ...
    def on_insert_text(self, buffer, location, text, length):
        # Apply pending tags to newly inserted text
        if self.pending_tags:
            start = location.copy()
            start.backward_chars(len(text))
            for tag_name in self.pending_tags:
    ...
    def create_new_document(self):
        ...
        self.modified = False
        self.win.set_title("Rich Text Editor")
        self.pending_tags = set()  # Clear pending tags on new document
    ...
    def load_file(self, file):
        ...
        try:
            ...
            if success:
                ...
                self.pending_tags = set()  # Clear pending tags when loading a file
        except GLib.Error as error:
    ...
    def on_bold_clicked(self, button):
        if button.get_active():
            self.pending_tags.add("bold")
        else:
            self.pending_tags.discard("bold")
    ...
    def on_italic_clicked(self, button):
        if button.get_active():
            self.pending_tags.add("italic")
        else:
            self.pending_tags.discard("italic")
    ...
    def on_underline_clicked(self, button):
        if button.get_active():
            self.pending_tags.add("underline")
        else:
            self.pending_tags.discard("underline")
    ...
    def on_font_changed(self, dropdown, param):
        ...
        if font_index < len(font_names):
            ...
            # Add to pending tags
            self.pending_tags.discard("font")
            self.pending_tags.add(tag_name)
    ...
    def on_font_size_changed(self, dropdown, param):
        ...
        if size_index < len(font_sizes):
            ...
            if bounds:
                ...
                self.buffer.apply_tag(tag, start, end)
            # Add to pending tags
            self.pending_tags.discard("size")
            self.pending_tags.add(tag_name)
