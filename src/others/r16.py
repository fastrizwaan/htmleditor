...
class PaginatedEditor(Adw.Application):
    ...
    def on_activate(self, app):
        ...
        self.width_adjustment = Gtk.Adjustment(
            value=40,  # Default width (in characters)
            lower=10,
            upper=100,
            step_increment=5,
            page_increment=10
    ...
    <script>
        ...
        function updatePages() {
            ...
            for (let i = 0; i < numPages; i++) {
                ...
                pageContent.textContent = pageText;
                ...
                try {
                    ...
                    if (newPageIndex < allPages.length) {
                        ...
                        if (pageContent && pageContent.firstChild) {
                            // Set selection in the text node
                            const newRange = document.createRange();
                            const actualOffset = Math.min(remainingOffset, pageContent.textContent.length);
                            
                            // Find the appropriate text node and position within it
                            let currentNode = pageContent.firstChild;
                            let currentOffset = 0;
                            ...
                            while (currentNode) {
                                if (currentNode.nodeType === 3) { // Text node
                                    ...
                                    if (currentOffset + nodeLength >= actualOffset) {
                                        // This is the node where our cursor should be
                                        newRange.setStart(currentNode, actualOffset - currentOffset);
                                        break;
                                    }
                            ...
                            // If we couldn't find the exact position, just put the cursor at the start
                            if (!currentNode) {
                                newRange.setStart(pageContent, 0);
                            }
                            ...
                            // If there's no text node, just focus the content div
                            pageContent.focus();
                        }
                    }
                } catch (e) {
        ...
        function handleKeyDown(e) {
            ...
            if (e.key === 'Enter') {
                // Let the browser handle the Enter naturally
                // This will create a proper line break in contenteditable
                
                // We'll need to update our text model after the input event
                setTimeout(() => {
                    // Update text model from the DOM
                    text = '';
                    document.querySelectorAll('.page-content').forEach(content => {
                        text += content.textContent;
                    });
                    
                    // In the next tick, update pagination
                    setTimeout(() => {
                        updatePages();
                        notifyContentChanged();
                    }, 0);
            ...
            if (e.key === 'ArrowDown' || e.key === 'PageDown') {
                ...
                if (isAtStart) {
                    ...
                    if (prevPage) {
                        ...
                        if (prevPage.lastChild && prevPage.lastChild.nodeType === 3) {
                            range.setStart(prevPage.lastChild, prevPage.lastChild.textContent.length);
                        } else {
                            range.setStart(prevPage, prevPage.childNodes.length);
                        }
        ...
        function isAtStartOfContent(element) {
            ...
            return range.startOffset === 0 && range.startContainer === element.firstChild;
        }
        ...
        function isAtEndOfContent(element) {
            ...
            if (range.startContainer.nodeType === 3) { // Text node
                return range.startOffset === range.startContainer.textContent.length &&
                       range.startContainer === element.lastChild;
            } else {
...
"""
    ...
    def on_save_dialog_response(self, dialog, response):
        if response == Gtk.ResponseType.ACCEPT:
            ...
            # Choose appropriate save method based on selected filter
            if file_filter.get_name() == "Text files":
                self.webview.evaluate_javascript("getContentAsText();", -1, None, None, None, None, self.save_text_callback, file_path)
            else:
                # Default to HTML for HTML filter and All files
