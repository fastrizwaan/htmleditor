...
################
    ...
    def get_editor_html(self, content=""):
        ...
        """##################
    # 1. First, let's add a checkbox button to the file toolbar
    # Add this to the create_file_toolbar method, in the insert_group section
    ...
    def create_file_toolbar(self, win):
        ...
        # Insert checkbox button (NEW)
        checkbox_button = Gtk.Button(icon_name="checkbox-symbolic") 
        # If checkbox-symbolic isn't available, you could use object-select-symbolic
        # If neither is available, consider using a custom icon or another standard icon
        checkbox_button.set_size_request(40, 36)
        checkbox_button.set_tooltip_text("Insert Checkbox")
        ...
        insert_group.append(image_button)
        insert_group.append(link_button)
        insert_group.append(checkbox_button)  # Add the new checkbox button
    ...
    # 2. Add the checkbox handler method

    def on_insert_checkbox_clicked(self, win, btn):
        """Handle checkbox insertion button click"""
        win.statusbar.set_text("Inserting checkbox...")
        
        # Execute JavaScript to insert a checkbox list item at the cursor position
        js_code = """
        (function() {
            insertCheckbox();
            return true;
    ...
    # 3. Add the JavaScript function for checkbox insertion
    # This should be added to the get_editor_js method

    def insert_checkbox_js(self):
        """JavaScript for checkbox insertion and related functionality"""
        return """
        // Checkbox unicode characters
        const CHECKBOX_UNCHECKED = "\u2610"; // ☐ BALLOT BOX
        const CHECKBOX_CHECKED = "\u2611";   // ☑ BALLOT BOX WITH CHECK
        
        // Function to insert checkbox at cursor position
        function insertCheckbox() {
            // Get the current selection
            const selection = window.getSelection();
            const range = selection.getRangeAt(0);
            
            // Get the current font size and font family
            const computedStyle = window.getComputedStyle(document.getElementById('editor'));
            const fontSize = computedStyle.fontSize || '11pt';
            const fontFamily = computedStyle.fontFamily || 'Sans';
            
            // Create a new list item with checkbox 
            let listItem = document.createElement('li');
            listItem.className = 'checkbox-item';
            listItem.setAttribute('data-checkbox-state', 'unchecked');
            
            // Create a span to hold the checkbox
            let checkboxSpan = document.createElement('span');
            checkboxSpan.className = 'checkbox-marker';
            checkboxSpan.textContent = CHECKBOX_UNCHECKED;
            checkboxSpan.style.fontFamily = fontFamily;
            checkboxSpan.style.fontSize = fontSize;
            checkboxSpan.style.marginRight = '5px';
            checkboxSpan.style.cursor = 'pointer';
            
            // Make checkbox clickable to toggle state
            checkboxSpan.addEventListener('click', function(e) {
                e.preventDefault();
                e.stopPropagation();
                
                const listItem = this.parentNode;
                const currentState = listItem.getAttribute('data-checkbox-state');
                
                if (currentState === 'checked') {
                    listItem.setAttribute('data-checkbox-state', 'unchecked');
                    this.textContent = CHECKBOX_UNCHECKED;
                } else {
                    listItem.setAttribute('data-checkbox-state', 'checked');
                    this.textContent = CHECKBOX_CHECKED;
                }
                
                try {
                    window.webkit.messageHandlers.contentChanged.postMessage("changed");
                } catch(e) {
                    console.log("Could not notify about checkbox change:", e);
                }
            });
            
            // Content span for the text
            let contentSpan = document.createElement('span');
            contentSpan.className = 'checkbox-content';
            contentSpan.textContent = 'Checkbox item';
            ...
            // Check if we're inside a list already
            let parentList = null;
            let currentNode = range.startContainer;
            
            // Look for parent list
            ...
            if (!parentList) {
                ...
                parentList.className = 'checkbox-list';
                
                // Insert the new list at the current position
                range.deleteContents();
                range.insertNode(parentList);
                
                // Add a paragraph after the list for easier editing
            ...
            // Position cursor after the checkbox item
            ...
            try {
                ...
                console.log("Could not notify about checkbox insertion:", e);
            }
        }
        
        // Convert selected text to checkbox items
        function convertToCheckboxList() {
            const selection = window.getSelection();
            ...
            for (const line of lines) {
                ...
                contentSpan.className = 'checkbox-content';
                contentSpan.textContent = line.trim();
                contentSpan.style.fontFamily = fontFamily;
                contentSpan.style.fontSize = fontSize;
            ...
            // Replace the selected content with the new list
            range.deleteContents();
            ...
            try {
                ...
                console.log("Could not notify about content conversion:", e);
            }
        }
        
        // Add listener for double-click on list items to convert them to checkboxes
        document.addEventListener('DOMContentLoaded', function() {
            const editor = document.getElementById('editor');
            
            editor.addEventListener('dblclick', function(e) {
                // Check if we double-clicked on a list item
                let target = e.target;
                while (target && target !== editor) {
                    if (target.tagName === 'LI' && !target.classList.contains('checkbox-item')) {
                        // Convert this regular list item to a checkbox item
                        convertListItemToCheckbox(target);
                        break;
                    }
        ...
        // Function to convert a regular list item to a checkbox item
        function convertListItemToCheckbox(listItem) {
            if (!listItem || listItem.classList.contains('checkbox-item')) return;
            ...
            try {
                ...
                console.log("Could not notify about conversion:", e);
            }
        }
        """

    # 4. Update the get_editor_js method to include the checkbox JavaScript
    # Find where get_editor_js is defined and update it to include our new function
    ...
    # 5. Update the CSS styles in get_editor_html function to enhance checkbox styling
    # Find the get_editor_html function and add this CSS to the <style> section
