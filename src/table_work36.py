...
###################
    def table_row_column_js_fixed(self):
        ...
        var resizeStartHeight = 0;
        var resizeStartWidth = 0;
        var resizeStartTableWidth = 0;
        var resizeStartTableHeight = 0;
        ...
        function updateColumnWidth(tableElement, columnIndex, newWidth) {
            ...
            const oldWidth = tableElement.rows[0].cells[columnIndex].offsetWidth;
            ...
            let currentTableWidth = tableElement.offsetWidth;
            tableElement.style.width = (currentTableWidth + widthDiff) + 'px';
            
            // Ensure all other columns maintain their width
            for (let j = 0; j < tableElement.rows[0].cells.length; j++) {
                if (j !== columnIndex) {
                    const cell = tableElement.rows[0].cells[j];
                    const currentWidth = cell.offsetWidth;
                    
                    // Apply the current width to all cells in this column
                    for (let i = 0; i < tableElement.rows.length; i++) {
                        const rowCell = tableElement.rows[i].cells[j];
                        if (rowCell) {
                            rowCell.style.width = currentWidth + 'px';
                            rowCell.style.minWidth = minWidth + 'px';
                            rowCell.style.maxWidth = currentWidth + 'px';
                        }
                    }
                }
            }
        ...
        function resizeTableFromEdge(tableElement, edge, deltaX, deltaY) {
            ...
            switch(edge) {
                case 'right':
                    const newWidth = resizeStartTableWidth + deltaX;
                    tableElement.style.width = Math.max(minWidth, newWidth) + 'px';
                    break;
                case 'bottom':
                    const newHeight = resizeStartTableHeight + deltaY;
                    tableElement.style.height = Math.max(minHeight, newHeight) + 'px';
                    break;
                case 'left':
                    const newWidthLeft = resizeStartTableWidth - deltaX;
                    if (newWidthLeft >= minWidth) {
                        tableElement.style.width = newWidthLeft + 'px';
                        
                        // Only adjust position if table is floating or has position absolute
                        if (tableElement.style.position === 'absolute' || tableElement.classList.contains('floating-table')) {
                            tableElement.style.left = (resizeStartLeft + deltaX) + 'px';
                        }
                    }
                    break;
                case 'top':
                    const newHeightTop = resizeStartTableHeight - deltaY;
                    if (newHeightTop >= minHeight) {
                        tableElement.style.height = newHeightTop + 'px';
                        
                        // Only adjust position if table is floating or has position absolute
                        if (tableElement.style.position === 'absolute' || tableElement.classList.contains('floating-table')) {
                            tableElement.style.top = (resizeStartTop + deltaY) + 'px';
                        }
                    }
                    break;
            }
        ...
        // Function to store initial column widths
        function storeInitialColumnWidths(tableElement) {
            initialColumnWidths = [];
            if (tableElement && tableElement.rows.length > 0) {
                const firstRow = tableElement.rows[0];
                for (let i = 0; i < firstRow.cells.length; i++) {
                    initialColumnWidths.push(firstRow.cells[i].offsetWidth);
                }
            }
        }
        ...
        document.addEventListener('mousedown', function(e) {
            if (e.target.classList.contains('column-resizer')) {
                ...
                if (table) {
                    ...
                    storeInitialColumnWidths(table);
                    
                    // Set table layout to fixed immediately when starting resize
                    table.style.tableLayout = 'fixed';
                    
                    // Lock the widths of all columns
                    for (let j = 0; j < table.rows[0].cells.length; j++) {
                        const currentCell = table.rows[0].cells[j];
                        const currentWidth = currentCell.offsetWidth;
                        for (let i = 0; i < table.rows.length; i++) {
                            const rowCell = table.rows[i].cells[j];
                            if (rowCell) {
                                rowCell.style.width = currentWidth + 'px';
                                rowCell.style.minWidth = '30px';
                                rowCell.style.maxWidth = currentWidth + 'px';
                            }
                        }
                    }
                ...
                resizeStartX = e.clientX;
                resizeStartY = e.clientY;
                resizeStartTableWidth = table.offsetWidth;
                resizeStartTableHeight = table.offsetHeight;
                resizeStartLeft = parseInt(table.style.left) || 0;
                resizeStartTop = parseInt(table.style.top) || 0;
    ...
    def table_row_column_js_fixed(self):
        ...
        function initializeTableResizers(tableElement) {
            ...
            // Add column resizers to ALL cells, including the last column
            for (let i = 0; i < tableElement.rows.length; i++) {
                const row = tableElement.rows[i];
                for (let j = 0; j < row.cells.length; j++) {
                    const cell = row.cells[j];
                    ...
                    // Mark if this is the last column
                    if (j === row.cells.length - 1) {
                        resizer.dataset.isLastColumn = 'true';
                    }
        ...
        function updateColumnWidth(tableElement, columnIndex, newWidth) {
            ...
            // If this is the last column, resize the table width instead
            const isLastColumn = columnIndex === tableElement.rows[0].cells.length - 1;
            
            if (isLastColumn) {
                // For the last column, resize the entire table
                const oldWidth = tableElement.rows[0].cells[columnIndex].offsetWidth;
                const widthDiff = newWidth - oldWidth;
                ...
                // For other columns, resize normally
                const oldWidth = tableElement.rows[0].cells[columnIndex].offsetWidth;
                const widthDiff = newWidth - oldWidth;
        ...
        function resizeTableFromEdge(tableElement, edge, deltaX, deltaY) {
            ...
            const minHeight = 50;
            const currentTableWidth = tableElement.offsetWidth;
            const currentTableHeight = tableElement.offsetHeight;
            ...
            switch(edge) {
                ...
                case 'left':
                    ...
                    // Only adjust position if the new width would be >= minimum
                    if (newWidthLeft >= minWidth) {
                        tableElement.style.width = newWidthLeft + 'px';
                        ...
                        // Set to minimum width without moving
                        tableElement.style.width = minWidth + 'px';
                    }
                    break;
                case 'top':
                    const newHeightTop = resizeStartTableHeight - deltaY;
                    
                    // Only adjust position if the new height would be >= minimum
                    if (newHeightTop >= minHeight) {
                        tableElement.style.height = newHeightTop + 'px';
                        ...
                        // Set to minimum height without moving
