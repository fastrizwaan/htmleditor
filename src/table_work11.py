...
################
    def insert_table_js(self):
            ...
            function setTableFloating() {
                ...
                activeTable.style.top = `${Math.max(topPos, editorScrollTop)}px`;
                activeTable.style.left = `${Math.max(leftPos, 0)}px`;
                
                // Enhance the drag handle for position control
                enhanceTableDragHandles(activeTable);
