...
################
    ...
    def create_table_toolbar(self, win):
        ...
        # Margin label
        margin_label = Gtk.Label(label="Margins:")
        margin_label.set_margin_end(5)
        toolbar.append(margin_label)
        
        # Create margin controls box
        margin_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=1)
        margin_box.add_css_class("linked")
        
        # Create margin spin buttons
        margin_controls = {}
        for side in ['top', 'right', 'bottom', 'left']:
            # Create a vertical box for label and spin button
            side_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=2)
            
            # Add label
            label = Gtk.Label(label=side.capitalize()[0])  # First letter (T, R, B, L)
            ...
            margin_box.append(side_box)
            margin_controls[side] = spin
        
        # Store margin controls in win object for later reference
        win.margin_controls = margin_controls
        
        toolbar.append(margin_box)
