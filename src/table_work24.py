...
################
    ...
    def _reset_default_colors(self, win):
        ...
        # Update color buttons to default values
        rgba = Gdk.RGBA()
        rgba.parse('#ccc' if not self._is_dark_theme() else '#444')
        win.table_props_popover.color_box.border_color_button.set_rgba(rgba)
    ...
    def _create_color_tab(self, win, popover):
        """Create the color properties tab with existing color initialization"""
        ...
        cell_color_label = Gtk.Label(label="Current Cell Color:")
    ...
    def _apply_cell_color(self, win, color_button):
        ...
        js_code = f"""
        (function() {{
            const result = setCellBackgroundColor('{hex_color}');
            return result;
        }})();
        """
        
        def handle_result(webview, result, data):
            try:
                js_result = webview.evaluate_javascript_finish(result)
                result_value = js_result.get_js_value().to_boolean() if hasattr(js_result, 'get_js_value') else js_result.to_boolean()
                
                if result_value:
                    win.statusbar.set_text(f"Applied cell color: {hex_color}")
                else:
                    win.statusbar.set_text("No cell selected")
            except:
                win.statusbar.set_text("Failed to apply cell color")
        
        win.webview.evaluate_javascript(js_code, -1, None, None, None, handle_result, None)    
            ...
            if result_str:
                ...
                if notebook:
                    ...
                    if properties.get('border'):
                        ...
                        border_color = properties['border'].get('color', '')
                    ...
                    if properties.get('colors'):
                        ...
                        if colors.get('background'):
                            rgba = self._parse_color_string(colors['background'])
                            if rgba:
                        ...
                        if colors.get('cell'):
                            rgba = self._parse_color_string(colors['cell'])
                            if rgba:
