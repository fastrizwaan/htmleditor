        ...
        function insertTable(rows, cols, hasHeader, borderWidth, tableWidth, isFloating) {
            ...
            if (hasHeader) {
                ...
                for (let j = 0; j < cols; j++) {
                    // Create cell style with or without border
                    let cellStyle = 'padding: 5px; background-color: ' + headerBgColor + ';';
                    if (borderWidth > 0) {
                        cellStyle += ' border: ' + borderWidth + 'px solid ' + borderColor + ';';
                    }
                    tableHTML += '<th style="' + cellStyle + '">Header ' + (j+1) + '</th>';
                }
                tableHTML += '</tr>';
            ...
            for (let i = 0; i < rows; i++) {
                ...
                for (let j = 0; j < cols; j++) {
                    // Create cell style with or without border
                    let cellStyle = 'padding: 5px; min-width: 30px;';
                    if (borderWidth > 0) {
                        cellStyle += ' border: ' + borderWidth + 'px solid ' + borderColor + ';';
                    }
...
############################
        ...
        function insertTable(rows, cols, hasHeader, borderWidth, tableWidth, isFloating) {
            ...
            setTimeout(() => {
                ...
                if (newTable) {
                    ...
                    if (isFloating) {
                        newTable.classList.add('floating-table');
                        setTableFloating(newTable, borderWidth);  // Pass border width to floating table function
                    }
    ...
    def table_floating_js(self):
        ...
        return """
            // Function to make a table floating (freely positionable)
            function setTableFloating(tableElement, borderWidth) {
                if (!tableElement && activeTable) {
                    tableElement = activeTable;
                }
                ...
                // Apply border conditionally based on borderWidth
                if (borderWidth === undefined) {
                    // Keep existing border if borderWidth wasn't specified
                } else if (borderWidth > 0) {
                    // Apply visible border with specified width
                    tableElement.style.border = borderWidth + 'px solid ' + getBorderColor() + ' !important';
                } else {
                    // Remove border completely when borderWidth is 0
                    tableElement.style.border = 'none !important';
                    tableElement.style.boxShadow = 'none'; // Remove shadow too for clean borderless look
                }
