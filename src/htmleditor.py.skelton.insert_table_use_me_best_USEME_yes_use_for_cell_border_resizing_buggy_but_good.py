...
#########################################################
    def insert_table_js(self):
        ...
        function setTableAlignment(alignClass) {
            ...
            // Set width to auto except for full-width
            if (alignClass === 'no-wrap') {setTableAlignment
                activeTable.style.width = '100%';
            } else {
                activeTable.style.width = 'auto';
            }
...
// Fix for alignment buttons not working - update the setTableAlignment function in insert_table_js
...
function setTableAlignment(alignClass) {
    ...
    if (alignClass === 'no-wrap') {
        ...
        // For other alignments, we still need to maintain the width
        // but not force it to 100%
        if (!activeTable.style.width || activeTable.style.width === '100%') {
            activeTable.style.width = 'auto';
        }
    }
...
// Updated addTableResizeHandles function with top and bottom border resize

function addTableResizeHandles(tableElement) {
    if (!tableElement) return;
    
    // Clear existing resize handles first
    clearTableResizeHandles(tableElement);
    ...
    for (let i = 0; i < cells.length - 1; i++) {
        ...
        handle.setAttribute('data-col-index', i);
        handle.setAttribute('contenteditable', 'false');
        handle.setAttribute('unselectable', 'on');
        
        // Position the handle at the right border of the cell
        handle.style.left = (cellRect.right - tableRect.left - 3) + 'px';
        handle.style.top = (tableRect.top - window.scrollY) + 'px';
        handle.style.height = tableRect.height + 'px';
        
        // Add event listener directly to the handle
        handle.addEventListener('mousedown', function(e) {
            e.preventDefault();
            e.stopPropagation();
            startColumnResize(e, tableElement, parseInt(this.getAttribute('data-col-index')));
        });
    ...
    for (let i = 0; i < rows.length - 1; i++) {
        ...
        handle.setAttribute('data-row-index', i);
        handle.setAttribute('contenteditable', 'false');
        handle.setAttribute('unselectable', 'on');
        
        // Position the handle at the bottom border of the row
        handle.style.top = (rowRect.bottom - tableRect.top - 3) + 'px';
        handle.style.left = (tableRect.left - window.scrollX) + 'px';
        ...
        handle.addEventListener('mousedown', function(e) {
            e.preventDefault();
            e.stopPropagation();
            startRowResize(e, tableElement, parseInt(this.getAttribute('data-row-index')));
        });
        
        tableElement.appendChild(handle);
    }
    
    // Add top border resize handle
    const topHandle = document.createElement('div');
    topHandle.className = 'top-resize-handle';
    topHandle.setAttribute('contenteditable', 'false');
    topHandle.setAttribute('unselectable', 'on');
    
    // Position at the top of the table
    topHandle.style.top = '0px';
    topHandle.style.left = '0px';
    topHandle.style.width = tableRect.width + 'px';
    
    // Add event listener for top resizing
    topHandle.addEventListener('mousedown', function(e) {
        e.preventDefault();
        e.stopPropagation();
        startTopResize(e, tableElement);
    });
    
    tableElement.appendChild(topHandle);
    
    // Add bottom border resize handle
    const bottomHandle = document.createElement('div');
    bottomHandle.className = 'bottom-resize-handle';
    bottomHandle.setAttribute('contenteditable', 'false');
    bottomHandle.setAttribute('unselectable', 'on');
    
    // Position at the bottom of the table
    bottomHandle.style.bottom = '0px';
    bottomHandle.style.left = '0px';
    bottomHandle.style.width = tableRect.width + 'px';
    
    // Add event listener for bottom resizing
    bottomHandle.addEventListener('mousedown', function(e) {
        e.preventDefault();
        e.stopPropagation();
        startBottomResize(e, tableElement);
    });
    
    tableElement.appendChild(bottomHandle);
}

// Functions to handle top and bottom border resizing
function startTopResize(e, tableElement) {
    e.preventDefault();
    isResizing = true;
    isTopResizing = true;
    activeTable = tableElement;
    resizeTarget = tableElement;
    dragStartY = e.clientY;
    
    // Get initial table position and height
    const tableRect = tableElement.getBoundingClientRect();
    tableStartY = tableRect.height;
    tableStartTop = tableRect.top;
    
    document.body.style.cursor = 'row-resize';
}

function startBottomResize(e, tableElement) {
    e.preventDefault();
    isResizing = true;
...
// Update the resizeTable function to handle top and bottom border resizing
function resizeTable(e) {
    if (!isResizing || !activeTable) return;
    ...
    if (isTopResizing) {
        const deltaY = e.clientY - dragStartY;
        const newHeight = Math.max(20, tableStartY - deltaY);
        
        // Adjust table height from the top
        activeTable.style.height = newHeight + 'px';
        
        // Update resize handles
        updateResizeHandles(activeTable);
        return;
    }
    ...
    if (isBottomResizing) {
        ...
        // Adjust table height from the bottom
        activeTable.style.height = newHeight + 'px';
    ...
    // Handle overall table resize (via corner handle)
    const deltaX = e.clientX - dragStartX;
    const deltaY = e.clientY - dragStartY;
    ...
    // Update resize handles when table size changes
    updateResizeHandles(activeTable);
}

// Update the mouseup handler to reset these new variables
document.addEventListener('mouseup', function() {
    if (isDragging || isResizing) {
        isDragging = false;
        isResizing = false;
        ...
        isRowResizing = false;
        isTopResizing = false;    // Reset the top resizing flag
        isBottomResizing = false; // Reset the bottom resizing flag
        document.body.style.cursor = '';
...
var isTopResizing = false;
var isBottomResizing = false;
var tableStartTop = 0;
        """
    ...
    #####################
    def insert_text_box_js(self):
        """JavaScript for insert text box with fixed initial size but still resizable"""
        return """
        // Function to insert a text box (a styled 1x1 table)
        function insertTextBox() {
            // Create the HTML for a 1x1 table with text box styling and initial dimensions
            let textBoxHTML = '<table class="text-box-table" border="1" cellspacing="0" cellpadding="5" ' +
                              'style="border-collapse: collapse; width: 80px; height: 80px; ' +
                              'border: 1px solid #ccc; resize: both; overflow: auto; margin: 10px 0;">' +
                              '<tr><td style="padding: 5px; vertical-align: top;">Type here...</td></tr>' +
                              '</table><p></p>';
            
            // Insert the text box table
            document.execCommand('insertHTML', false, textBoxHTML);
            
            // Activate the newly inserted table/text box to give it handles
            setTimeout(() => {
                const tables = document.querySelectorAll('table.text-box-table');
                const newTable = tables[tables.length - 1];
                if (newTable) {
        ...
        // Function to handle text box selection without preventing resize from handles
        // We'll modify the document event listeners to handle this
        document.addEventListener('DOMContentLoaded', function() {
            const editor = document.getElementById('editor');
            
            // Prevent text selection from causing table resize by tracking selection events
            editor.addEventListener('mousedown', function(e) {
                // For table handles, handle separately
                if (e.target.classList.contains('table-handle') || 
                    e.target.classList.contains('table-drag-handle')) {
                    // This is already handled by the existing table code
                    return;
                }
                
                // Find if we're in a textbox cell
                let cell = e.target;
                if (cell.nodeType === 3) { // Text node
                    cell = cell.parentNode;
                }
                
                // Check if this is a textbox table cell
                let isInTextBoxCell = false;
                let textBoxTable = null;
                
                // Traverse up to find if we're in a text box table cell
                while (cell && cell !== editor) {
                    if (cell.tagName === 'TD' || cell.tagName === 'TH') {
                        const table = findParentTable(cell);
                        if (table && table.classList.contains('text-box-table')) {
                            isInTextBoxCell = true;
                            textBoxTable = table;
                            break;
                        }
                    }
                ...
                // Save original dimensions when selecting text in a text box
                if (isInTextBoxCell) {
                    // Track clicks for triple-click detection
                    const now = new Date().getTime();
                    let lastClick = parseInt(textBoxTable.getAttribute('data-last-click') || 0);
                    let clickCount = parseInt(textBoxTable.getAttribute('data-click-count') || 0);
                    
                    if (now - lastClick < 400) { // 400ms threshold for multi-clicks
                        clickCount++;
                    } else {
                        clickCount = 1;
                    }
                    
                    textBoxTable.setAttribute('data-last-click', now);
                    textBoxTable.setAttribute('data-click-count', clickCount);
                    
                    // On triple-click (or when a selection might occur), save original dimensions
                    if (clickCount >= 2) { // Start tracking from double-click
                        // Store current dimensions
                        textBoxTable.setAttribute('data-original-width', textBoxTable.style.width || window.getComputedStyle(textBoxTable).width);
                        textBoxTable.setAttribute('data-original-height', textBoxTable.style.height || window.getComputedStyle(textBoxTable).height);
                        
                        // Store that we're in text selection mode, to differentiate from resize handle interaction
                        textBoxTable.setAttribute('data-text-selection', 'true');
                    }
                }
            });
            
            // After selection changes, check if we need to restore dimensions
            document.addEventListener('selectionchange', function() {
                const tables = document.querySelectorAll('table.text-box-table[data-text-selection="true"]');
                tables.forEach(table => {
                    // If table is in text selection mode
                    if (table.getAttribute('data-original-width')) {
                        // Restore dimensions, but only if not resizing via handle
                        if (!isResizing) {
                            table.style.width = table.getAttribute('data-original-width');
                            table.style.height = table.getAttribute('data-original-height');
                        }
                    }
                });
            });
            
            // When mouse is released, end text selection mode
            document.addEventListener('mouseup', function() {
                const tables = document.querySelectorAll('table.text-box-table[data-text-selection="true"]');
                tables.forEach(table => {
                    // Only clear the selection flag, keep dimensions unless resizing occurred
                    table.removeAttribute('data-text-selection');
                    
                    // If not in a resize operation, also clear the saved dimensions
                    if (!isResizing) {
                        table.removeAttribute('data-original-width');
                        table.removeAttribute('data-original-height');
                    }
                });
            });
            
            // Extend the existing resize start function to handle this
            const originalStartTableResize = startTableResize;
            startTableResize = function(e, tableElement) {
                // Clear any text selection tracking on resize
                if (tableElement && tableElement.classList.contains('text-box-table')) {
                    tableElement.removeAttribute('data-text-selection');
                    tableElement.removeAttribute('data-original-width');
                    tableElement.removeAttribute('data-original-height');
                }
                
                // Call the original function
                originalStartTableResize(e, tableElement);
            };
        });
        
        // Extend activateTable to ensure minimum size for text boxes
        const originalActivateTable = activateTable;
        activateTable = function(tableElement) {
            // Call the original activateTable function first
            originalActivateTable(tableElement);
            
            // Apply additional styling if this is a text box
            if (tableElement.classList.contains('text-box-table')) {
                // Ensure minimum dimensions (but don't force fixed size)
                if (!tableElement.style.width || 
                    parseInt(tableElement.style.width) < 80) {
                    tableElement.style.width = '80px';
                }
                if (!tableElement.style.height || 
                    parseInt(tableElement.style.height) < 80) {
                    tableElement.style.height = '80px';
                }
                
                // Add a class to indicate this is an active text box
                tableElement.classList.add('active-text-box');
                
                // Try to notify that a text box was activated
                try {
                    window.webkit.messageHandlers.textBoxActivated.postMessage('textbox-activated');
                } catch(e) {
                    // Just ignore if the handler isn't available
                }
            }
        };
        """
...
################################################]]
    def insert_table_js(self):
        """JavaScript for insert table and related functionality, with column and row resizing"""
        return """
        // Function to insert a table at the current cursor position
        function insertTable(rows, cols, hasHeader, borderWidth, tableWidth) {
            // Create table HTML
            ...
            // Add class and style attributes
            tableHTML += 'class="resizable-table no-wrap" style="border-collapse: collapse; width: ' + tableWidth + ';">';
            ...
            setTimeout(() => {
                ...
                if (newTable) {
                    ...
                    // Add column and row resize handles
                    addTableResizeHandles(newTable);
                }
            }, 10);
        }
        ...
        // Function to add column and row resize handles to a table
        function addTableResizeHandles(tableElement) {
            if (!tableElement) return;
        ...
        // Function to clear column and row resize handles
        function clearTableResizeHandles(tableElement) {
            if (!tableElement) return;
            
            const colHandles = tableElement.querySelectorAll('.col-resize-handle');
            const rowHandles = tableElement.querySelectorAll('.row-resize-handle');
            
            colHandles.forEach(handle => handle.remove());
            rowHandles.forEach(handle => handle.remove());
        }
        
        // Function to start column resize
        function startColumnResize(e, tableElement, columnIndex) {
            e.preventDefault();
            isResizing = true;
            ...
            for (let i = 0; i < firstRow.cells.length; i++) {
                ...
                initialSizes.push(cell.offsetWidth);
            }
            
            document.body.style.cursor = 'col-resize';
        }
        
        // Function to start row resize
        function startRowResize(e, tableElement, rowIndex) {
            e.preventDefault();
            isResizing = true;
            isRowResizing = true;
            activeTable = tableElement;
            ...
            const rows = tableElement.rows;
            for (let i = 0; i < rows.length; i++) {
                initialSizes.push(rows[i].offsetHeight);
            }
        ...
        function resizeColumn(e) {
            if (!isColumnResizing || !resizeTarget) return;
            
            const deltaX = e.clientX - dragStartX;
            const rows = resizeTarget.rows;
            if (rows.length === 0) return;
            
            // Calculate new column width, ensuring it doesn't go below minimum
            const newWidth = Math.max(30, initialSizes[resizeIndex] + deltaX);
            const nextColWidth = Math.max(30, initialSizes[resizeIndex + 1] - deltaX);
            
            // Apply new widths to all cells in the affected columns
            for (let i = 0; i < rows.length; i++) {
                if (resizeIndex < rows[i].cells.length) {
                    rows[i].cells[resizeIndex].style.width = newWidth + 'px';
                    
                    // If this is not the last column, adjust the next column's width as well
                    if (resizeIndex + 1 < rows[i].cells.length) {
                        rows[i].cells[resizeIndex + 1].style.width = nextColWidth + 'px';
                    }
                }
            }
            
            // Update column resize handles
            updateResizeHandles(resizeTarget);
        }
        
        // Function to resize row
        function resizeRow(e) {
            if (!isRowResizing || !resizeTarget) return;
            
            const deltaY = e.clientY - dragStartY;
            const rows = resizeTarget.rows;
            
            // Calculate new row height, ensuring it doesn't go below minimum
            const newHeight = Math.max(20, initialSizes[resizeIndex] + deltaY);
            
            // Apply new height to the row
            rows[resizeIndex].style.height = newHeight + 'px';
            
            // Update row resize handles
            updateResizeHandles(resizeTarget);
        }
        
        // Function to update resize handles after resizing
        function updateResizeHandles(tableElement) {
            if (!tableElement) return;
            
            // Clear and re-add all resize handles
            clearTableResizeHandles(tableElement);
            addTableResizeHandles(tableElement);
        }
        ...
        function activateTable(tableElement) {
            ...
            alignmentClasses.forEach(cls => tableElement.classList.remove(cls));
            tableElement.classList.add(currentAlignment);
            tableElement.classList.add('resizable-table'); // Ensure the table has the resizable class
        ...
        function deactivateAllTables() {
            ...
            tables.forEach(table => {
                ...
                // Clear column and row resize handles
                clearTableResizeHandles(table);
            });
        ...
        function addTableRow(tableElement, position) {
            ...
            if (rows.length > 0) {
                ...
                for (let i = 0; i < rows[0].cells.length; i++) {
                    ...
                    // Copy width from the corresponding cell in the first row
                    if (rows[0].cells[i].style.width) {
                        cell.style.width = rows[0].cells[i].style.width;
                    }
                }
            }
            
            // Update resize handles after adding a row
            updateResizeHandles(tableElement);
        ...
        function addTableColumn(tableElement, position) {
            ...
            // Update resize handles after adding a column
            updateResizeHandles(tableElement);
        ...
        // Rest of the existing table functions (deleteRow, deleteColumn, etc.)
        // ...
    ...
    def get_editor_html(self, content=""):
        """Return HTML for the editor with improved table styles including column and row resize handles"""
        content = content.replace('\\', '\\\\').replace('"', '\\"').replace('\n', '\\n')
        return f"""
        <!DOCTYPE html>
        <html style="height: 100%;">
...
/* Add these CSS styles to your get_editor_html method */
...
@media (prefers-color-scheme: dark) {{
    ...
    .col-resize-handle:hover, .row-resize-handle:hover,
    .top-resize-handle:hover, .bottom-resize-handle:hover {{
        background-color: rgba(0, 120, 215, 0.5);
    }}
}}
            </style>
