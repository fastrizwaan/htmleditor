...
## Insert related code
    def insert_table_js(self):
        """JavaScript for insert table with border-only dragging and cell-level text selection"""
        return """
        /**
         * JavaScript for table insertion with border-only dragging and cell text selection
         */
        function insertTable(rows, cols, hasHeader, borderWidth, width) {
            ...
            setTimeout(() => {
                ...
                if (newTable) {
                    ...
                    // Add cell-level selection to all cells
                    setupCellLevelSelection(newTable);
                }
            }, 10);
        }
        ...
        function initializeTable(table) {
            ...
            document.addEventListener('mousemove', (e) => {
                if (isDragging && activeTable === table) {
                    const dx = e.clientX - dragStartX;
                    const dy = e.clientY - dragStartY;
                    table.style.left = (originalLeft + dx) + 'px';
                    table.style.top = (originalTop + dy) + 'px';
                } else if (isResizing && activeTable === table) {
                    const dx = e.clientX - dragStartX;
                    const dy = e.clientY - dragStartY;
                    table.style.width = (originalWidth + dx) + 'px';
                    table.style.height = (originalHeight + dy) + 'px';
                }
            });
