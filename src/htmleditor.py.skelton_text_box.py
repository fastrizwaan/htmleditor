...
####################.
    def on_content_changed(self, win, manager, result):
        """Handle content changed events from the webview"""
        # Mark the document as modified
        win.modified = True
        
        # Update window title to show modified status
        if win.current_file:
            base_name = os.path.basename(win.current_file)
            win.title_widget.set_title(f"{base_name}* - HTML Editor")
        else:
            win.title_widget.set_title("Untitled* - HTML Editor")
        
        # Update status bar
        win.statusbar.set_text("Document modified")
        
        # If auto-save is enabled, schedule a save
        if win.auto_save_enabled and win.current_file:
            ...
            win.auto_save_source_id = GLib.timeout_add_seconds(
                ...
                lambda: self.auto_save_document(win)
            )
    def insert_text_box_js(self):
        ...
        function insertTextBox() {
            // Create a styled div to serve as a text frame
            let textBox = '<div contenteditable="true" class="text-frame" ' +
            ...
            // Focus on the newly inserted text frame
            setTimeout(() => {
                ...
                if (newFrame) {
                    ...
                    // Notify that a text frame was added
                    try {
                        window.webkit.messageHandlers.contentChanged.postMessage('text-frame-added');
                    } catch(e) {
                        // Handler not implemented yet, that's okay
                        console.log("Message handler for text frame not available");
                    }
                    
                    // Make the frame editable and selectable
                    setupFrameInteractions(newFrame);
                }
            }, 10);
        }
        
        // Function to set up interactions for text frames
        function setupFrameInteractions(frame) {
            // Allow the frame to be selected as a whole by clicking its border
            frame.addEventListener('mousedown', function(e) {
                ...
                if (isNearBorder) {
                    ...
                    // Add a class to show it's selected
                    frame.classList.add('frame-selected');
                    
                    // Remove the selection class from all other frames
                    document.querySelectorAll('div.text-frame.frame-selected').forEach(f => {
                        if (f !== frame) f.classList.remove('frame-selected');
                    });
                }
            });
            
            // Handle click inside the frame normally
            frame.addEventListener('click', function(e) {
                // Allow normal editing inside the frame
                e.stopPropagation();
            });
        }
        
        // Function to detect and setup all text frames in the document
        function setupAllTextFrames() {
            document.querySelectorAll('div.text-frame').forEach(frame => {
                if (!frame.hasAttribute('data-setup')) {
                    setupFrameInteractions(frame);
                    frame.setAttribute('data-setup', 'true');
                }
            });
        }
        
        // Make sure to setup frames when the document loads
        document.addEventListener('DOMContentLoaded', function() {
        ...
        // Call this after loading content to ensure all frames are interactive
        function initTextFrames() {
            setupAllTextFrames();
        }
        """
