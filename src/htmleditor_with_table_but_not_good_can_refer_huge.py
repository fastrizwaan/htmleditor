...
dropdown.flat, dropdown.flat button { background: none; border-radius: 3px; }
...
"""
    ...
    def create_window(self):
        ...
        try:
            ...
            # Register formatting changed handler
...
##########
    ...
    # Also update table_handles_css_js to ensure proper display properties
...
##############
    # Update init_editor_js to handle Tab navigation in tables

    def init_editor_js(self):
        """JavaScript to initialize the editor and set up event listeners with table tab navigation"""
        return """
        document.addEventListener('DOMContentLoaded', function() {
            const editor = document.getElementById('editor');
            ...
            // Function to focus and position caret in a cell without selecting text
            function focusCell(cell) {
                const range = document.createRange();
                const sel = window.getSelection();
                
                // Find the last text node in the cell to position cursor at the end
                function findLastTextNode(node) {
                ...
                // Position caret at the end of the cell without selecting text
                if (cell.childNodes.length > 0) {
                    const lastTextNode = findLastTextNode(cell);
                    if (lastTextNode) {
                        // Position at the end of the text node
                        range.setStart(lastTextNode, lastTextNode.length);
                        range.setEnd(lastTextNode, lastTextNode.length);
                    } else {
                        // If no text node, position at the end of the cell
                        range.selectNodeContents(cell);
                        range.collapse(false); // Collapse to end
                    }
                } else {
                    // Empty cell - position at the beginning
    ...
    # Also update table_event_handlers_js to ensure handle visibility is managed during tab navigation
...
#######
    ...
    def init_editor_js(self):
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // IMPORTANT: Add table styles immediately on DOMContentLoaded
            addTableHandleStyles();
    ...
    def table_insert_functions_js(self):
        ...
        // Function to insert a table at the current cursor position
        function insertTable(rows, cols, hasHeader, borderWidth, tableWidth, isFloating) {
            console.log('Inserting table:', rows, cols, hasHeader, borderWidth, tableWidth, isFloating);
            ...
            tableHTML += 'style="border-collapse: collapse; width: ' + tableWidth + '; margin: 6px 6px 0 0; border: ' + borderWidth + 'px solid ' + borderColor + ';">';
            ...
            setTimeout(() => {
                ...
                // Set default margins
                tableElement.style.marginTop = '6px';
                tableElement.style.marginRight = '6px';
                tableElement.style.marginBottom = '0px';
                tableElement.style.marginLeft = '0px';
                
                // Store margin values as attributes
                tableElement.setAttribute('data-margin-top', '6');
                tableElement.setAttribute('data-margin-right', '6');
                tableElement.setAttribute('data-margin-bottom', '0');
                tableElement.setAttribute('data-margin-left', '0');
                
                // Make table floating if requested
                if (isFloating) {
                    tableElement.classList.add('floating-table');
                    setTableFloating(tableElement);
                }
    ...
    def table_theme_helpers_js(self):
        ...
        // Function to get appropriate border color based on current theme
        function getBorderColor() {
            // For now, always return a visible color
            return '#999'; 
        }
    ...
    def on_table_dialog_response(self, win, dialog, rows, cols, has_header, border_width, width_option, is_floating):
        ...
        # Execute JavaScript to insert the table - using document.execCommand for better compatibility
        js_code = f"""
        (function() {{
            // Get theme-appropriate colors
            const borderColor = '#ccc';  // Simple fallback color
            const headerBgColor = '#f0f0f0';  // Simple fallback color
            
            // Create table HTML
            let tableHTML = '<table border="1" cellspacing="0" cellpadding="5" style="border-collapse: collapse; width: {width_value}; margin: 6px 6px 0 0;">';
            
            // Create header row if requested
            if ({str(has_header).lower()}) {{
                tableHTML += '<tr>';
                for (let j = 0; j < {cols}; j++) {{
                    tableHTML += '<th style="border: {border_width}px solid ' + borderColor + '; padding: 5px; background-color: ' + headerBgColor + ';">Header ' + (j+1) + '</th>';
                }}
                tableHTML += '</tr>';
            }}
            ...
            for (let i = 0; i < {rows - (1 if has_header else 0)}; i++) {{
                tableHTML += '<tr>';
                for (let j = 0; j < {cols}; j++) {{
                    tableHTML += '<td style="border: {border_width}px solid ' + borderColor + '; padding: 5px; min-width: 30px;">Cell</td>';
                }}
                tableHTML += '</tr>';
            }}
            ...
            // Find the table we just inserted and activate it
            setTimeout(() => {{
                const tables = document.querySelectorAll('table');
                ...
                if (newTable) {{
                    ...
                    // Activate the table
                    if (typeof activateTable === 'function') {{
                        activateTable(newTable);
                    }}
