...
##########
    # Update table_event_handlers_js to hide handles during cell editing

    def table_event_handlers_js(self):
        """JavaScript for table event handlers with handle hiding during editing"""
        return """
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            editor.addEventListener('keydown', function(e) {
                const cell = findParentCell(e.target);
                if (cell && activeTable && activeTable.contains(cell)) {
                    // Hide handles when typing in cells (except for navigation keys)
...
##############
    ...
    def init_editor_js(self):
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Function to focus and select content in a cell
    ...
    def table_event_handlers_js(self):
        ...
        """            
###################
    # Updated init_editor_js with better focus management and key handling

    def init_editor_js(self):
        """JavaScript to initialize the editor with improved table navigation and key handling"""
        return """
        document.addEventListener('DOMContentLoaded', function() {
            const editor = document.getElementById('editor');
            ...
            function findNextCell(currentCell) {
                ...
                for (let i = 0; i < row.cells.length; i++) {
                    const newCell = newRow.insertCell(i);
                    // Set initial content with a space to ensure proper rendering
                    newCell.innerHTML = '&nbsp;';
                    // Copy styles from the last row
                    const lastRowCell = table.rows[rowIndex].cells[i];
                    if (lastRowCell) {
            ...
            function focusCell(cell) {
                ...
                // Get the cell's text content (if any)
                const textContent = cell.textContent.trim();
                
                if (textContent && textContent !== '' && textContent !== '\u00A0') {
                    // If there's actual text content, select it
                    ...
                    if (firstTextNode && lastTextNode) {
                        ...
                        // Fallback: just place cursor at the start
                        range.selectNodeContents(cell);
                        range.collapse(true);
                    }
                } else {
                    // If no text content, just place cursor at the beginning
                    range.selectNodeContents(cell);
                    range.collapse(true);
                }
            ...
            editor.addEventListener('keydown', function(e) {
                ...
                if (e.key === 'Tab') {
                    ...
                    if (cell && (cell.tagName === 'TD' || cell.tagName === 'TH')) {
                        e.preventDefault();
                        e.stopPropagation(); // Prevent focus from moving to toolbar
                        ...
                        return false; // Ensure the event is fully blocked
                    }
            ...
            // Handle Delete/Backspace keys to prevent structure deletion
            editor.addEventListener('keydown', function(e) {
                if (e.key === 'Delete' || e.key === 'Backspace') {
                    ...
                    if (cell && (cell.tagName === 'TD' || cell.tagName === 'TH')) {
                        // If we have a selection that spans multiple cells, prevent default
                        const range = selection.getRangeAt(0);
                        
                        // Check if selection crosses cell boundaries
                        let startCell = range.startContainer;
                        while (startCell && startCell.tagName !== 'TD' && startCell.tagName !== 'TH') {
                            startCell = startCell.parentElement;
                        }
                        
                        let endCell = range.endContainer;
                        while (endCell && endCell.tagName !== 'TD' && endCell.tagName !== 'TH') {
                            endCell = endCell.parentElement;
                        }
                        
                        if (startCell !== endCell || startCell === cell.parentElement) {
                            // Selection crosses cells or includes the cell itself
                            e.preventDefault();
                            
                            // Delete only text content, not structure
            ...
            // Handle Enter key to prevent structure deletion
            editor.addEventListener('keydown', function(e) {
                if (e.key === 'Enter') {
                    const selection = window.getSelection();
                    ...
                    if (cell && (cell.tagName === 'TD' || cell.tagName === 'TH')) {
                        // If the selection includes more than just text, prevent default
                        const range = selection.getRangeAt(0);
                        ...
                        if (startCell !== endCell || startCell === cell.parentElement) {
                            ...
                            // If there's a selection, delete it and insert a line break
                            if (!selection.isCollapsed) {
                                range.deleteContents();
                            }
            ...
            // Prevent focus from leaving editor on Shift+Tab
            editor.addEventListener('keydown', function(e) {
                if (e.key === 'Tab' && e.shiftKey) {
                    ...
                    if (selection.rangeCount > 0) {
                        ...
                        // Check if we're at the beginning of the editor
                        if (startNode === editor || 
                            (startNode === editor.firstChild && range.startOffset === 0)) {
                            e.preventDefault();
                            e.stopPropagation();
                        }
                    }
